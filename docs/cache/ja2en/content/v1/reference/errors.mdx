...
:   One of the following status codes:
    ...
    - `3154`: Indicates that an error occurred in the "close" event handler registered by the WebSocket engine. In most cases, a `StateTransitionError` occurs during the transition from the closing state to the closed state.
