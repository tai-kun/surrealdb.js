...
A type that represents the event name and the arguments (array) the event takes as a key-value pair.
...
`.on` is a function that registers an event listener to the event emitter. If you try to register the same function multiple times to the same event, registration will be ignored except for the first time.
...
The event listener to register. This function receives the same arguments as the task runner of `TaskQueue`, and then receives the arguments passed to `.emit` in a variable-length manner.
...
The event listener to unregister. If you omit this, all event listeners registered to the target `event` will be unregistered.
...
`.once` is similar to `.on`, but it catches the event only once. Also, instead of a callback function, you receive the value with the return value `StatefulPromise`.
