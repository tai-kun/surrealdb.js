...
The above order indicates the priority of conversion. For example, even if a plain object has a `.toJSON` function, if it also has a `.toSurql` function, the `.toSurql` function will be prioritized.

[`is-plain-obj`](https://github.com/sindresorhus/is-plain-obj) is used to determine if a plain object. Therefore, if a plain object does not have `.toSurql` or `.toJSON` and has `Symbol.toStringTag` or `Symbol.iterator`, it will not be recognized as an object and an error will be thrown.
