...
`Duration` is a class that represents a duration of time.
...
A pair of seconds and nanoseconds. If the preset is `standard`, more values can be passed to the `Duration` object.

`number` type input is calculated in milliseconds. This is intended for use cases where you want to create a `Duration` object from the difference between two `Date` objects:
...
`bigint` type input is calculated in nanoseconds. This is intended for use cases where you want to create a `Duration` object from high-precision time, such as nanoseconds returned by [`process.hrtime.bigint()`](https://nodejs.org/api/process.html#processhrtimebigint):
...
`string` type input must be a string representation of the duration. The string representation of the duration is a format in which a set of numbers and units is concatenated. For example, `1h4h30m20s1350ms` means "1 day and 4 hours 30 minutes, 21 seconds 350 milliseconds." The available units are as follows:
...
The seconds portion of the duration. Read-only for presets less than `standard`. The value is an integer between 0 and 2<sup>64</sup>-1.
...
The nanoseconds portion of the duration. Read-only for presets less than `standard`. The value is an integer between 0 and 999999999.
...
Maps the duration to each unit. The value of each unit is greater than or equal to 0 and less than the maximum value of that unit. Note that if the duration is 0, an object with only the `nanoseconds` property having a value of `0` is returned.
...
Returns the result of mapping the duration to each unit. If the value of the unit is 0, the property is omitted, but if the duration is 0, an object with only `nanoseconds` having a value of `0` is returned.
...
Gets the string representation of the duration in ISO format.
...
Returns the string representation of the duration in ISO format.
...
Adds the specified number of years from the duration.
...
Adds the specified number of weeks from the duration.
...
Adds the specified number of days from the duration.
...
Adds the specified number of hours from the duration.
...
Adds the specified number of minutes from the duration.
...
Adds the specified number of seconds from the duration.
...
Adds the specified number of milliseconds from the duration.
...
Adds the specified number of microseconds from the duration.
...
Adds the specified number of nanoseconds from the duration.
