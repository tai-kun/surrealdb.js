...
A pair of minimum and maximum values for the range. If it is unlimited, it is `null`.
...
Converts the `Range` object to a string that can be used in SurrealQL.
...
#### General-Purpose `Range` Object Determination

It is recommended to use the `isRange` function to determine if a value is a `Range` object without relying on presets. This function does not use `instanceof`, so it is useful when the preset of the value being verified is unknown.
