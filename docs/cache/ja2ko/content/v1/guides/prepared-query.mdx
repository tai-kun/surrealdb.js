...
쿼리를 미리 준비하면 타입 추론이 가능합니다.  그러나 추론되는 타입은 수동으로 설정해야 합니다.

## 쿼리 결과 검증

`.returns` 의 인수에 쿼리 결과 검증기(validator)를 전달하면 쿼리가 더 안전해집니다. 다음 예제에서는 zod를 사용하여 쿼리 결과가 의도한 값인지 검증합니다.
...
zod에 의존하지 않고, valibot 또는 일반적인 함수를 사용하여 응답을 검증할 수 있습니다.
...
위 예제에서 전송되는 RPC 요청의 매개변수는 다음과 같습니다.
...
슬롯은 반드시 변수 이름이 필요합니다. `.type()` 메서드로 변수에 타입 레벨에서 제약을 설정할 수 있습니다. `.returns()`와 마찬가지로 `.type()` 의 인수에 값을 검증하는 함수를 전달할 수 있습니다. 위 예제에서는 변수 이름 `id`의 슬롯은 테이블 이름이 `"user"`인 레코드 ID여야 합니다.

슬롯의 인수는 변수 이름에 이어 기본값을 설정할 수 있습니다. 실행 시 슬롯의 변수 이름이 생략되면 이 기본값이 사용됩니다. 위 예제에서는 변수 이름 `age`의 슬롯에 기본값 `42`가 설정되었습니다.

슬롯은 `.type()` 외에도 `.rename()`과 `.default()`, `.optional()`, `.required()`를 사용할 수 있습니다.

미리 정의된 쿼리에 실행 시 변수 지정이 필수인 슬롯이 포함된 경우, 해당 슬롯이 지정되지 않으면 쿼리는 실행 시 `SurrealTypeError`로 거부됩니다. TypeScript에서 올바르게 타입 추론이 되면 실행하지 않고도 타입 레벨에서 오류가 표시됩니다. 예를 들어, 다음과 같이 필수 변수를 생략하고 `tsc`로 타입 검사를 수행하면 오류가 발생합니다.
