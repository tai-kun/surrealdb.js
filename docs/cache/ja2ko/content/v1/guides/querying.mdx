...
SurrealQL 실행 결과의 배열로 해결되는 `Promise` 객체를 반환합니다. 하나 이상의 문이 실패하면 해당 오류 메시지를 포함하는 `QueryFailedError`로 거부합니다.

이 메서드가 거부되면 실행 결과를 얻을 수 있는 방법이 없습니다. 오류 처리를 수동으로 수행하려면 [`.queryRaw()`](#queryraw)를 사용할 수 있습니다.

#### 예

다음 예에서는 쿼리 실행에 성공합니다.
...
다음 예에서는 한쪽 문이 실패하기 때문에 쿼리는 `QueryFailedError`로 거부됩니다.
