...
따옴표로 묶인 문자열을 반환합니다. 묶는 데 사용되는 따옴표는 제공된 문자열에 작은따옴표 (`'`) 가 포함되어 있는지에 따라 다릅니다.

- 제공된 문자열이 비어 있는 경우
  - `''` 를 반환합니다.
- 제공된 문자열에 작은따옴표가 **없는** 경우
  - 작은따옴표로 묶습니다.
- 제공된 문자열에 작은따옴표가 **있는** 경우
  - 큰따옴표로 묶습니다.
- 제공된 문자열에 작은따옴표와 큰따옴표가 모두 포함되어 있는 경우
  - 큰따옴표로 묶습니다.
  - 큰따옴표는 백슬래시 (`\`) 로 이스케이프합니다.

#### 예

다음 예에서는 작은따옴표가 **없는** 문자열을 작은따옴표로 묶습니다.
...
다음 예에서는 작은따옴표가 **있는** 문자열을 큰따옴표로 묶습니다.
...
다음 예에서는 작은따옴표와 큰따옴표가 모두 포함된 문자열을 큰따옴표로 묶고, 필요한 경우 이스케이프합니다.
...
`escapeKey` 는 [`quoteStr`](/ko/reference/utils/quote-str/) 와 비슷하지만, 객체의 키에 대해 사용하는 함수입니다. 편의상 빈 문자열도 따옴표로 묶습니다.
...
이스케이프된 문자열을 반환합니다. 필요한 경우 큰따옴표로 묶습니다.

- 제공된 문자열이 비어 있는 경우
  - `""` 를 반환합니다.
- 제공된 문자열이 영숫자와 밑줄로만 구성된 경우
  - 그대로 반환합니다.
- 제공된 문자열에 영숫자와 밑줄 이외의 문자가 포함되어 있는 경우
  - 큰따옴표로 묶습니다.
- 제공된 문자열에 백슬래시 (`\`) 가 포함되어 있는 경우
  - 이스케이프합니다.
- 제공된 문자열에 큰따옴표가 포함되어 있는 경우
  - 이스케이프합니다.
...
다음 예에서는 문자열을 큰따옴표로 묶고, 필요한 경우 이스케이프합니다.
...
`escapeRid` 는 [`quoteStr`](/ko/reference/utils/quote-str/) 와 비슷하지만, 레코드 ID에 대해 사용하는 함수입니다. 편의상 빈 문자열도 따옴표로 묶습니다.
...
이스케이프된 문자열을 반환합니다. 필요한 경우 괄호 (`⟨⟩`) 로 묶습니다.

- 제공된 문자열이 비어 있는 경우
  - `⟨⟩` 를 반환합니다.
- 제공된 문자열이 숫자로 시작하지 않고, 영숫자와 밑줄로만 구성된 경우
  - 그대로 반환합니다.
- 제공된 문자열에 괄호 (`⟩`) 가 포함되어 있는 경우
  - 이스케이프합니다.
- 제공된 문자열이 숫자로 시작하거나, 영숫자와 밑줄 이외의 문자가 포함되어 있는 경우
  - 괄호 (`⟨⟩`) 로 묶습니다.
...
`escapeIdent` 는 [`quoteStr`](/ko/reference/utils/quote-str/) 와 비슷하지만, 네임스페이스, 데이터베이스, 테이블, 필드의 이름에 대해 사용하는 함수입니다. 편의상 빈 문자열도 따옴표로 묶습니다.
...
이스케이프된 문자열을 반환합니다. 필요한 경우 백틱 (<code>`</code>) 으로 묶습니다.

- 제공된 문자열이 비어 있는 경우
  - <code>``</code> 를 반환합니다.{/* TODO(tai-kun): " になる問題解決する */}
- 제공된 문자열이 숫자로 시작하지 않고, 영숫자와 밑줄로만 구성된 경우
  - 그대로 반환합니다.
- 제공된 문자열에 백틱 (<code>`</code>) 가 포함되어 있는 경우
  - 이스케이프합니다.
- 제공된 문자열이 숫자로 시작하거나, 영숫자와 밑줄 이외의 문자가 포함되어 있는 경우
...
이전 예에서 사용된 문자열에는 JavaScript에서 본 문자열 길이와 사람이 본 문자열 길이가 다른 문자가 포함되어 있습니다.

| 종류 | 표시 | `.length` | `Uint8Array` |
| --- | --- | --- | --- |
| ASCII | a | 1 | `[ 0x61 ]` |
| 서로게이트 쌍 | 𩸽 | 2 | `[ 0xF0, 0xA9, 0xB8, 0xBD ]` |
| 변형 선택기 | 👍🏽 | 4 | `[ 0xF0, 0x9F, 0x91, 0x8D, 0xF0, 0x9F, 0x8F, 0xBD ]` |
| 결합 문자 | パ | 2 | `[ 0xE3, 0x83, 0x8F, 0xE3, 0x82, 0x9A ]` |

`.length` 의 값이 직관과 다를 경우, 문자열에 대한 연산을 주의 깊게 생각해야 합니다.
