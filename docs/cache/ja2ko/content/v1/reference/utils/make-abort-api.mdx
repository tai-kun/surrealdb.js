...
취소 신호에 `"abort"` 이벤트를 전송합니다. 옵션으로 하나의 인수 `reason`을 받을 수 있으며, `"abort"` 이벤트를 통해 취소 이유를 전달할 수 있습니다.
...
위의 예는 `new AbortController()`와 거의 동일한 의미를 갖습니다. `AbortController`와 다른 점은 `makeAbortApi`가 객체가 아닌 배열을 반환하는 것입니다.

다음 예에서는 연결할 취소 신호를 지정하여 Abort API를 만듭니다.
