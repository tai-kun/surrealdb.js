...
<Aside type="caution">
  빌드된 SDK를 JavaScript에서 사용하는 경우에는 문제가 없지만, 이 SDK의 소스 코드(TypeScript)를 직접 사용하는 경우에는 스테이지 3 데코레이터 지원이 필요합니다.
</Aside>
...
다음 예에서는 `mutex` 유무에 따라 비동기 처리의 사냥 타이밍이 다르다는 것을 보여줍니다.
