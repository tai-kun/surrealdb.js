...
`StatefulPromise`는 처리되지 않은 상태로 거부되더라도 오류를 던지지 않는 `PromiseLike`를 구현하는 클래스입니다.
...
`new`를 통해 호출되면 `StatefulPromise`는 해당 인스턴스를 반환합니다.

#### 예

##### 해결

다음 예에서는 `StatefulPromise`가 해결될 때까지 대기하고 결과를 받습니다.
...
다음 예에서는 `StatefulPromise`가 거부될 때까지 대기하고 처리하면 오류를 던지는 것을 기대합니다.
...
일반 `Promise`에서는 처리되지 않은 상태로 거부(즉, `reject`가 호출됨)되면 `unhandledrejection`이 발생하지만, `StatefulPromise`는 명시적으로 처리될 때까지 오류를 던지지 않는 것을 알 수 있습니다.

### `.state` <Badge variant="instance" /><Badge variant="property" /><Badge variant="readonly" />

현재 `StatefulPromise`의 상태를 나타냅니다. 상태에는 다음 3가지가 있습니다.

- `pending`: 보류 중임을 나타냅니다. 아직 해결되거나 거부되지 않았습니다.
...
인수 `value`로 해결된 `StatefulPromise` 객체를 반환합니다. 값이 `StatefulPromise` 객체이면 해당 값을 반환합니다.
...
인수 `reason`으로 거부된 `StatefulPromise`를 반환합니다.

### `.withResolvers()` <Badge variant="static" /><Badge variant="method" />

`.withResolvers`는 새로운 `StatefulPromise` 객체와 해당 객체를 해결하거나 거부하는 함수가 포함된 객체를 반환하는 함수입니다.
...
호출되는 함수입니다. 이 함수는 `StatefulPromise`가 아닌 값을 반환할 수 있습니다. 또한 함수 내에서 동기적으로 오류를 던질 수 있습니다.

#### 반환 값

- `func`가 동기적으로 값을 반환하면 해결된 상태의 `StatefulPromise`를 반환합니다.
- `func`가 동기적으로 오류를 던지면 거부된 상태의 `StatefulPromise`를 반환합니다.
- `func`가 비동기적으로 해결되거나 거부되면 보류 상태의 `StatefulPromise`를 반환합니다.

#### 예

다음 예는 동기적으로 던져진 오류가 `StatefulPromise`에 의해 잡히는 것을 보여줍니다.
...
반복 가능한 객체입니다. `StatefulPromise` 객체 이외의 객체를 포함할 수도 있지만, 해당 객체는 오류로 판단되지 않습니다. 또한 일반 `Promise`도 마찬가지로 무시됩니다.
