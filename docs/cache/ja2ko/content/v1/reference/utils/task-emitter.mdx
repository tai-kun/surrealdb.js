...
이벤트 이름과 이벤트가 받는 인수 (배열)를 키-값으로 표현한 타입입니다.
...
`.on`은 이벤트 에미터에 이벤트 리스너를 등록하는 함수입니다. 동일한 이벤트에 동일한 함수를 여러 번 등록하려고 하면 처음 1회를 제외하고는 등록이 무시됩니다.
...
발송할 이벤트 리스너에 대한 값입니다.

#### 반환 값

이벤트 리스너가 없으면 `undefined`를 반환합니다. 이벤트 리스너가 있으면 해당 리스너가 해결/거부될 때까지 대기하는 `StatefulPromise` 배열을 반환합니다. 일반적으로 이 `StatefulPromise`를 처리할 필요는 없습니다. 이벤트로 인해 발생하는 부작용을 현재 컨텍스트에서 대기해야 하거나 이벤트 리스너의 해결/거부에 관심이 있는 경우에 사용할 수 있습니다.
