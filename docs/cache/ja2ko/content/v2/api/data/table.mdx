...
`Table`은 레코드 ID의 테이블 이름을 명시적으로 표현하는 클래스입니다.
...
프리셋에 의존하지 않고 값이 `Table` 객체인지 판단하려면 `isTable` 함수를 사용하는 것이 좋습니다. 이 함수는 `instanceof`를 사용하지 않으므로 검증할 값의 프리셋이 불명확한 경우에 유용합니다.
