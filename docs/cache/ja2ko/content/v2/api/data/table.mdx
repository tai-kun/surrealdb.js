...
`new`를 통해 호출될 때 `Table`은 해당 인스턴스를 반환합니다.
...
`Table` 객체를 SurrealQL에서 변수로 사용할 수 있는 문자열로 변환합니다.
...
프리셋에 의존하지 않고 값이 `Table` 객체인지 판정하려면 `isTable` 함수를 사용하는 것이 좋습니다. 이 함수는 `instanceof`를 사용하지 않으므로 검증하는 값의 프리셋이 알 수 없는 경우에 유용합니다.
