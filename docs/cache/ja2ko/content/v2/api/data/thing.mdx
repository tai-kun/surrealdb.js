...
`Thing`은 [레코드 ID](https://surrealdb.com/docs/surrealql/datamodel/ids)를 표현하는 클래스입니다.
...
`new`를 통해 호출될 경우, `Thing`은 해당 인스턴스를 반환합니다.
...
`Thing` 객체를 SurrealQL의 변수에서 사용 가능한 문자열로 변환합니다. 테이블 이름과 ID 부분은 각각 SurrealDB의 구현과 동일하게 이스케이프되고 포맷됩니다. 또한 객체의 프로퍼티는 오름차순으로 정렬됩니다.
...
`Thing` 객체를 SurrealQL에 삽입할 수 있는 문자열로 변환합니다. `.toString()`과 유사하지만, `r` 접두사를 추가하여 쿼리 파서에게 문자열이 레코드 ID임을 알립니다.
...
새로운 `Thing` 객체를 반환합니다. `Thing` 클래스를 상속하고 있다면, 해당 클래스의 인스턴스를 반환합니다.
...
프리셋에 의존하지 않고 값이 `Thing` 객체인지 판단하려면 `isThing` 함수를 사용하는 것이 좋습니다. 이 함수는 `instanceof`를 사용하지 않으므로, 검증하려는 값의 프리셋이 불명확할 경우 유용합니다.
