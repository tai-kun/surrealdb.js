...
UUID의 바이트 배열 표현 또는 standard 프리셋인 경우 문자열입니다.
...
UUID의 변형입니다. 이는 UUID의 13번째 비트의 상위 비트로 정의되며, 다음 3가지 주요 변형과 미래를 위한 1가지 예약된 변형이 있습니다.
...
2. RFC 4122 호환
    - `0b10xx`
    - RFC 4122(UUID 표준 사양)를 기반으로 하는 UUID입니다.
...
이 속성은 위 변형 외에도 NIL UUID와 MAX UUID를 나타냅니다.
...
UUID의 변형이 `"RFC"`이고 버전이 v1, v6 또는 v7인 경우 시간 스탬프 부분입니다. 그렇지 않으면 `null`입니다. 편의상 시간 스탬프는 모두 UNIX 에포크부터의 밀리초로 계산됩니다.
...
`.valueOf()`와 `.toJSON()`은 이 메소드와 동일한 결과를 반환합니다.
...
둘 중 하나에 시간 스탬프 부분이 없거나 시간 스탬프가 동일하면 `0`을 반환합니다. 비교 대상 UUID의 시간 스탬프가 더 진행된 시간이면 `-1`, 더 이른 시간이면 `1`을 반환합니다.

#### 예

다음 예에서는 다른 UUID 버전의 시간 스탬프를 비교하여 오름차순으로 정렬합니다:
