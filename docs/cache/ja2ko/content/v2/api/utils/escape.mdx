...
따옴표로 묶인 문자열을 반환합니다. 묶는 데 사용되는 따옴표는 주어진 문자열에 작은따옴표(`'`)가 포함되어 있는지에 따라 다릅니다:

- 주어진 문자열이 비어 있는 경우
  - `''`를 반환합니다.
- 주어진 문자열에 작은따옴표가 **포함되지 않은** 경우
  - 작은따옴표로 묶습니다.
- 주어진 문자열에 작은따옴표가 **포함된** 경우
  - 큰따옴표로 묶습니다.
- 주어진 문자열에 작은따옴표와 큰따옴표가 모두 포함된 경우
  - 큰따옴표로 묶습니다.
  - 큰따옴표는 백슬래시(`\`)로 이스케이프합니다.

#### 예

다음 예에서는 작은따옴표가 **포함되지 않은** 문자열을 작은따옴표로 묶습니다:
...
다음 예에서는 작은따옴표가 **포함된** 문자열을 큰따옴표로 묶습니다:
...
다음 예에서는 작은따옴표와 큰따옴표가 모두 포함된 문자열을 큰따옴표로 묶고 필요하면 이스케이프합니다:
...
`escapeKey`는 [`quoteStr`](/v2/api/utils/quote-str/)와 비슷하지만 객체의 키에 대해 사용하는 함수입니다. 편의상 빈 문자열도 따옴표로 묶습니다.
...
`escapeRid`는 [`quoteStr`](/v2/api/utils/quote-str/)와 비슷하지만 레코드 ID에 대해 사용하는 함수입니다. 편의상 빈 문자열도 따옴표로 묶습니다.
...
`escapeIdent`는 [`quoteStr`](/v2/api/utils/quote-str/)와 비슷하지만 네임스페이스, 데이터베이스, 테이블, 필드 이름에 대해 사용하는 함수입니다. 편의상 빈 문자열도 따옴표로 묶습니다.
...
| 변이 문자 선택기 | 👍🏽 | 4 | `[ 0xF0, 0x9F, 0x91, 0x8D, 0xF0, 0x9F, 0x8F, 0xBD ]` |
