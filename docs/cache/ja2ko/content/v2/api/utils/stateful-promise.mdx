...
slug: ko/v2/api/utils/stateful-promise
---
...
`new`를 통해 호출될 때, `StatefulPromise`는 자신의 인스턴스를 반환합니다.

#### 예제

##### 해결

다음 예제에서는 `StatefulPromise`가 해결될 때까지 기다리고 그 결과를 받습니다:
...
다음 예제에서는 `StatefulPromise`가 거부될 때까지 기다리고, 처리하면 에러를 던지는 것을 예상합니다:
...
일반 `Promise`는 처리되지 않은 상태로 거부될 경우 (즉, `reject`가 호출될 경우) `unhandledrejection`이 발생하지만, `StatefulPromise`는 명시적으로 처리될 때까지 에러를 던지지 않는 것을 알 수 있습니다.

### `.state` <Badge variant="instance" /><Badge variant="property" /><Badge variant="readonly" />

현재 `StatefulPromise`의 상태를 나타냅니다. 상태는 다음 3가지가 있습니다:
...
`.then`은 `StatefulPromise`가 성공하거나 실패할 때 호출되는 콜백 함수를 받아 `StatefulPromise` 객체를 반환합니다.
...
[`Promise.prototype.then`](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise/then#onfulfilled)와 동일합니다.

#### 반환값

`StatefulPromise` 객체가 반환됩니다.
...
인수의 `value`로 해결된 `StatefulPromise` 객체를 반환합니다. 값이 `StatefulPromise` 객체이면 그 값을 반환합니다.
...
`.withResolvers`는 새로운 `StatefulPromise` 객체와 해결 또는 거부하는 함수를 포함한 객체를 반환하는 함수입니다.
...
호출될 함수입니다. 이 함수는 `StatefulPromise`가 아닌 값을 반환할 수 있습니다. 또한 함수 내에서 동기적으로 에러를 던질 수 있습니다.

#### 반환값

- `func`가 동기적으로 값을 반환하면 해결된 상태의 `StatefulPromise`를 반환합니다.
- `func`가 동기적으로 에러를 던지면 거부된 상태의 `StatefulPromise`를 반환합니다.
- `func`가 비동기적으로 해결 또는 거부되면 대기 상태의 `StatefulPromise`를 반환합니다.

#### 예제

다음 예제는 동기적으로 던져진 에러가 `StatefulPromise`에 의해 처리되는 것을 보여줍니다:
...
`.allRejected`는 거부된 `StatefulPromise`의 이유만 수집하는 함수입니다.
...
반복 가능한 객체입니다. `StatefulPromise` 객체가 아닌 것을 포함할 수도 있지만, 에러로 판단되지 않습니다. 또한 일반 `Promise`도 마찬가지로 무시됩니다.
