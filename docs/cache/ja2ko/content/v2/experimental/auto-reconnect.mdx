...
`initialDelay?: number`
:   첫 번째 재연결을 시도할 때까지의 지연 시간(밀리초)입니다. 기본값은 `500`입니다.

`maxDelay?: number`
:   재연결 지연 시간(밀리초)의 최대값입니다. 백오프가 진행되면 지연이 증가하지만, 이 값이 상한선입니다. 기본값은 `30_000`(30초)입니다.

`shouldReconnect?: { ping?: { threshold: number; perMillis: number }} | (error) => boolean`
:   재연결을 수행할 조건을 지정합니다. 함수로 지정한 경우, 오류 이벤트 인수를 기반으로 재연결을 수행할지 여부를 판단합니다.
...
:   `.phase`:
    - `"waiting"`    ... 초기 상태입니다. 재연결은 한 번도 수행되지 않았습니다.
    - `"pending"`    ... 재연결이 요청되었고, 실행될 때까지 일정 시간 대기하고 있습니다.
    - `"closing"`    ... 연결 끊는 중입니다.
    - `"connecting"` ... 연결 중입니다. 연결 끊기에 실패해도 연결을 시도합니다.
...
ar.on("success", (endpoint) => {
  console.log(`${endpoint}로 재연결에 성공했습니다 🎉`);
});

ar.on("failure", (endpoint, cause) => {
  console.error(`${endpoint}로 재연결에 실패했습니다 🤯`);
