...
SurrealDB 엔드포인트에 연결합니다. 이 메서드는 비동기적으로 실행되지만, 동일한 인스턴스 내에서 다른 `.connect()` 또는 `.close()`와 중복 실행될 수 없습니다.
...
`signal?: AbortSignal`
:   연결을 중단하기 위한 취소 신호입니다. 기본값은 15초 만료되는 취소 신호가 설정됩니다.
...
다음 예시는 WebSocket 프로토콜로 SurrealDB에 연결하는 방법입니다:
...
`.connect()`는 비동기 처리 컨텍스트에서 동시에 실행되지 않으므로, 다음 예시도 마찬가지로 연결에 실패합니다. 일반적으로 어떤 엔드포인트가 먼저 연결을 설정할지 알 수 없으므로, 에러 메시지는 매번 다를 수 있습니다:
...
SurrealDB와의 연결을 해제합니다. 아직 연결되지 않았거나 이미 연결이 해제된 경우에도 오류를 발생시키지 않습니다. 이 메서드는 비동기적으로 실행되지만, 동일한 인스턴스 내에서 `.connect()` 또는 다른 `.close()`와 중복 실행될 수 없습니다.
...
다음 예시는 WebSocket 프로토콜로 SurrealDB에 연결한 후, 연결을 해제하는 방법입니다:
...
연결 정보를 가져옵니다. 연결 정보는 항상 내부 정보가 복사된 값을 갖습니다. 따라서 연결 정보에는 `URL` 객체가 포함되지만, 이를 직접 변경하더라도 `Surreal` 객체가 내부적으로 유지하는 엔드포인트 정보는 변경되지 않습니다. 아직 연결이 요청되지 않은 경우 `undefined`입니다.
...
연결이 설정된 경우 연결 정보를 반환합니다. 그렇지 않으면 `undefined`를 반환합니다. 연결 정보는 다음 항목을 포함합니다.

`state: "connecting" | "open" | "closing" | "closed"`
:   현재 연결 상태를 나타내는 값입니다. 각 값은 다음과 같은 의미를 가집니다:

`endpoint: URL | null`
:   연결할 엔드포인트입니다. 연결 해제 상태일 때만 `null`입니다.
...
연결할 엔드포인트입니다. 연결 해제 상태일 때만 `null`입니다. 연결이 설정되지 않은 경우 `undefined`입니다.
...
`rpc_${RpcMethod}_${number}`
:   양방향 통신에서 RPC 응답이 발생할 때 발생하는 이벤트입니다. 현재 WebSocket 프로토콜에서만 사용되며, HTTP 프로토콜에서는 이 이벤트가 사용되지 않습니다. `RpcMethod`는 예를 들어 `ping` 또는 `use`와 같은 PRC 메서드 이름입니다. `number`는 양방향 통신에서 요청/응답을 연결하기 위한 식별자이며, 1부터 2^53-1까지의 정수 값을 가집니다.
