...
SurrealDB 엔드포인트에 연결합니다. 이 메서드는 비동기적으로 실행되지만, 동일한 인스턴스 내에서 다른 `.connect()` 또는 `.close()`와 중복해서 실행될 수 없습니다.
...
`signal?: AbortSignal`
:   연결을 중단하기 위한 중단 신호입니다. 기본값은 15초 타임아웃으로 설정된 중단 신호입니다.
...
다음 예시는 WebSocket 프로토콜로 SurrealDB에 연결하는 방법을 보여줍니다:
...
`.connect()`는 비동기 처리 컨텍스트에서 동시에 실행될 수 없기 때문에 다음 예시도 마찬가지로 연결에 실패합니다. 일반적으로 어떤 엔드포인트가 먼저 연결을 수립하는지 알 수 없기 때문에 에러 메시지는 경우에 따라 다를 수 있습니다:
...
SurrealDB와의 연결을 해제합니다. 아직 연결되지 않았거나, 이미 해제된 상태라도 에러를 발생시키지 않습니다. 이 메서드는 비동기적으로 실행되지만, 동일한 인스턴스 내에서 `.connect()` 또는 다른 `.close()`와 중복해서 실행될 수 없습니다.
...
`force?: boolean`
:   `true`로 설정하면 `Surreal` 객체의 이벤트 리스너로 전달되는 중단 신호에 중단 신호를 전송합니다. 이벤트 리스너는 처리가 완료되지 않더라도 즉시 중단될 것으로 예상됩니다.
...
다음 예시는 WebSocket 프로토콜로 SurrealDB에 연결한 후, 연결을 해제하는 방법을 보여줍니다:
...
연결 정보를 가져옵니다. 연결 정보는 항상 내부 정보를 복사한 값을 가지고 있습니다. 따라서 연결 정보에는 `URL` 객체가 포함되어 있지만, 이를 직접 수정해도 `Surreal` 객체가 내부적으로 가지고 있는 엔드포인트 정보는 변경되지 않습니다. 아직 연결 요청이 이루어지지 않았다면 `undefined`입니다.
...
연결이 수립된 경우 연결 정보를 반환합니다. 그렇지 않으면 `undefined`를 반환합니다. 연결 정보에는 다음 항목이 포함됩니다.

`state: "connecting" | "open" | "closing" | "closed"`
:   현재 연결 상태를 나타내는 값입니다. 각 값은 다음을 의미합니다:

`endpoint: URL | null`
:   연결 대상 엔드포인트입니다. 연결 해제 상태에서만 `null`입니다.
...
`token: string | null`
:   현재 인증된 토큰입니다. 회원 가입 또는 로그인을 하지 않은 경우 `null`입니다.

각 연결 상태에 따른 연결 정보의 가능한 값은 다음 표와 같습니다:
...
편의상 연결이 수립되지 않은 경우에도 `"closed"`(undefined 아님)입니다.
...
연결 대상 엔드포인트입니다. 연결 해제 상태에서만 `null`입니다. 연결이 수립되지 않은 경우 `undefined`입니다.
...
현재 선택된 네임스페이스입니다. 네임스페이스를 선택하지 않은 경우 `null`입니다. 연결이 수립되지 않은 경우 `undefined`입니다.
...
현재 선택된 데이터베이스 이름입니다. 데이터베이스를 선택하지 않은 경우 `null`입니다. 연결이 수립되지 않은 경우 `undefined`입니다.
...
현재 인증된 토큰입니다. 회원 가입 또는 로그인을 하지 않은 경우 `null`입니다. 연결이 수립되지 않은 경우 `undefined`입니다.
...
`rpc_${RpcMethod}_${number}`
:   양방향 통신에서 RPC 응답이 발생할 때 발생하는 이벤트입니다. `RpcMethod`는 예를 들어 `ping` 또는 `use`와 같은 RPC 메서드 이름입니다. `number`는 양방향 통신에서 요청/응답을 연결하기 위한 식별자이며, 1부터 2^53-1까지의 정수를 가집니다. HTTP에서는 0으로 고정됩니다.
...
이벤트가 발생했을 때 실행되는 함수입니다. [태스크 에미터](/v2/api/utils/task-emitter#on)도 참조하세요. `eventArgs`에 전달되는 값은 이벤트에 따라 다릅니다.
