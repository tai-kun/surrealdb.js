...
SurrealDB 엔드포인트를 URL로 지정합니다. URL 경로의 끝이 `/rpc`가 아니면 자동으로 추가됩니다. 따라서 `http://localhost:8000`을 인수로 전달하면 `http://localhost:8000/rpc`에 연결합니다.
...
`signal?: AbortSignal`
:   연결을 중단하기 위한 취소 신호입니다. 기본값은 15초 후에 타임아웃되는 취소 신호가 설정됩니다.
...
다음 예는 WebSocket 프로토콜로 SurrealDB에 연결합니다:
...
`.close()`로 연결을 끊지 않고 다른 엔드포인트에 연결할 수 없습니다. 이전에 연결이 설정된 엔드포인트와의 연결은 유지됩니다:
...
`.connect()`는 비동기 처리 컨텍스트에서 동시에 실행될 수 없으므로, 다음 예도 마찬가지로 연결에 실패합니다. 일반적으로 어떤 엔드포인트가 먼저 연결을 설정하는지 알 수 없으므로 에러 메시지는 매번 다를 수 있습니다:
...
SurrealDB와의 연결을 끊습니다. 아직 연결되지 않았거나 이미 연결이 끊어진 경우에도 에러를 발생시키지 않습니다. 이 메서드는 비동기적으로 실행되지만, 동일한 인스턴스 내에서 `.connect()` 또는 다른 `.close()`와 중복되어 실행되지는 않습니다.
...
`force?: boolean`
:   `true`로 설정하면 `Surreal` 객체의 이벤트 리스너에 전달되는 취소 신호에 취소 신호를 전송합니다. 이벤트 리스너는 처리가 완료되지 않았더라도 즉시 중단될 것으로 예상됩니다.
...
다음 예는 WebSocket 프로토콜로 SurrealDB에 연결한 후 연결을 끊습니다:
...
연결 정보를 가져옵니다. 연결 정보는 항상 내부 정보가 복사된 값을 가지고 있습니다. 따라서 연결 정보에는 `URL` 객체가 포함되어 있지만, 이를 직접 변경해도 `Surreal` 객체가 내부적으로 가지고 있는 엔드포인트 정보는 바뀌지 않습니다. 아직 연결 요청이 이루어지지 않은 경우 `undefined`입니다.
...
연결이 설정된 경우 연결 정보를 반환합니다. 그렇지 않은 경우 `undefined`를 반환합니다. 연결 정보에는 다음 항목이 포함됩니다.
...
편의상, 연결이 설정되지 않은 경우에도 `"closed"`가 됩니다 (`undefined`가 아닙니다.)
...
현재 선택된 데이터베이스 이름입니다. 접 데이터베이스를 선택하지 않은 경우 `null`입니다. 연결이 설정되지 않은 경우 `undefined`입니다.
...
`rpc_${RpcMethod}_${number}`
:   양방향 통신에서 RPC 응답이 발생하는 이벤트입니다. 현재 WebSocket 프로토콜에서만 사용되며 HTTP 프로토콜에서는 이 이벤트가 사용되지 않습니다. `RpcMethod`는 예를 들어 `ping` 또는 `use`와 같은 PRC 메서드 이름입니다. `number`는 양방향 통신에서 요청/응답을 연결하기 위한 식별자이며, 1에서 2^53-1까지의 정수를 취합니다.

`live_${string}`
:   라이브 쿼리의 이벤트를 수신했을 때 발생하는 이벤트입니다. 현재 WebSocket 프로토콜에서만 사용되며 HTTP 프로토콜에서는 이 이벤트가 사용되지 않습니다. `string`은 라이브 쿼리의 UUID입니다.
...
이벤트가 발생했을 때 실행되는 함수입니다. [작업 에미터](/v2/api/utils/task-emitter#on)도 참조하세요. `eventArgs`에 전달되는 값은 이벤트에 따라 다릅니다.
...
작업 리스너의 옵션입니다. [작업 에미터](/v2/api/utils/task-emitter#once)를 참조하세요.

#### 반환 값

`.on()`의 작업 리스너의 `eventArgs`로 해결되는 `StatefulPromise`를 반환합니다.
