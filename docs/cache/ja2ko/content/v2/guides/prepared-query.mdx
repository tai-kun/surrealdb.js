...
`.as`의 인수로 쿼리 결과의 검증기를 전달하면 쿼리는 더 안전해집니다. 다음 예에서는 zod를 사용하여 쿼리 결과가 의도한 값인지 검증합니다.
...
zod에 의존하고 있지 않으므로 valibot 또는 일반적인 함수로 응답을 검증할 수 있습니다.
...
슬롯에는 반드시 변수 이름이 필요합니다. `.as()` 메서드로 변수에 타입 레벨에서 제약을 설정할 수 있습니다. `.as()`와 마찬가지로 `.as()`의 인수로 값을 검증하는 함수를 전달할 수 있습니다. 위 예에서는 변수 이름 `id`의 슬롯은 테이블 이름이 `"user"`인 레코드 ID여야 합니다.

슬롯의 인수에는 변수 이름에 이어 기본값을 설정할 수 있습니다. 실행 시 슬롯의 변수 이름이 생략된 경우 이 기본값이 사용됩니다. 위 예에서는 변수 이름 `age`의 슬롯에는 기본값 `42`가 설정되어 있습니다.

슬롯은 `.as()` 외에도 `.rename()`과 `.default()`, `.optional()`, `.required()`를 사용할 수 있습니다.

미리 정의된 쿼리에 실행 시 변수 지정이 필수인 슬롯이 포함된 경우, 그것이 지정되지 않으면 쿼리는 실행 시 `SurrealTypeError`로 거부됩니다. TypeScript에서 제대로 타입 추론되면 실행하지 않고도 타입 레벨에서 오류가 표시됩니다. 예를 들어 다음과 같이 필수 변수를 생략하고 `tsc`로 타입 검사를 하면 오류가 발생합니다.
