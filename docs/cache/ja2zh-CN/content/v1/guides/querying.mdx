...
在 SurrealQL 执行时传递的变量。请注意，变量名不能以美元符号开头。
...
返回一个 `Promise` 对象，解析为 SurrealQL 执行结果的数组。如果语句失败，则会以包含这些错误信息的 `QueryFailedError` 拒绝。
...
以下示例演示了由于其中一个语句失败，查询以 `QueryFailedError` 拒绝：
...
在 SurrealQL 执行时传递的变量。请注意，变量名不能以 `$` 开头。
...
返回一个 `Promise` 对象，解析为 SurrealQL 执行结果的数组。
