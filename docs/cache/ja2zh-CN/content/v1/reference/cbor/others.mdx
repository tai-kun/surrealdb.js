...
`write` 是一个辅助函数，用于将所有可被编码为 CBOR 形式的 JavaScript 值写入 `Writer`。
...
如果值不是基本类型，并且具有 `toCBOR` 属性并且该属性是一个函数，则优先调用该函数。
...
:   编码器默认将大多数值编码为 CBOR，但对于一些特殊值，可能需要将其替换为可编码的值。这个选项可以通过指定一个函数或函数数组来实现替换处理。如果函数返回 `Symbol.for("@tai-kun/surrealdb/cbor/continue")` 获得的 `CONTINUE` 符号，则会调用后续函数。如果无法替换任何值，则会抛出 `SurrealTypeError`。

`isSafeMapKey?: (key: unknown, map: ReadonlyMap<unknown, unknown>) => boolean`
:   这个选项是一个用于验证 `Map` 对象键是否安全的函数。该函数接收两个参数：`key` 和 `map`。`key` 是键的值，`map` 是正在验证的 `Map` 对象。如果该函数返回 `false`，则会抛出 `CborUnsafeMapKeyError`。默认情况下，它会判定 `"__proto__"` 或 `"constructor"` 为不安全的键。请注意，指定此选项将完全覆盖默认验证。如果您想在默认验证的基础上添加自己的验证，则应该将 `"__proto__"` 或 `"constructor"` 也视为不安全的键。

`isSafeObjectKey?: (key: unknown, obj: { readonly [p: string]: unknown }) => boolean`
:   这个选项是一个用于验证普通对象键是否安全的函数。该函数接收两个参数：`key` 和 `obj`。`key` 是键的值，`obj` 是正在验证的普通对象。如果该函数返回 `false`，则会抛出 `CborUnsafeMapKeyError`。默认情况下，它会判定 `"__proto__"` 或 `"constructor"` 为不安全的键。请注意，指定此选项将完全覆盖默认验证。如果您想在默认验证的基础上添加自己的验证，则应该将 `"__proto__"` 或 `"constructor"` 也视为不安全的键。
...
数值。如果为整数，则有效值为 -2^53-1 到 2^53-1。在 JavaScript 中，没有方法来确定 `.0` 是否为浮点数，因此它将被视为整数并编码为 CBOR。例如，`-1.0` 等于 `-1`，它是一个整数。
...
`writeUtf8String` 是一个辅助函数，用于将 UTF-8 形式的字符串写入 `Writer`。
