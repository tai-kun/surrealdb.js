...
`SurrealError` 是此 SDK 明确抛出的大多数错误对象的基类。此类继承自 JavaScript 的 `Error` 类。此错误对象很少单独抛出，通常会抛出继承自它的具有特殊意义的错误对象。建议在 `try-catch` 语句中捕获此错误，以便轻松识别此 SDK 明确抛出的错误。
...
当检测到循环引用时抛出的错误。主要是在将 JavaScript 值转换为其他格式的过程中，当检测到与父对象相同的对象时抛出。
...
当到达不可到达的代码时抛出的错误。如果抛出此错误，则可能遇到了此 SDK 的错误。
...
在上面的例子中，虽然 https 协议的引擎指明了使用 http 协议的引擎，但 http 协议的引擎却试图使用 https 协议的引擎，因此抛出循环引用错误。

## EngineNotFoundError

继承自：`SurrealError`

当尝试使用未设置引擎的协议连接时抛出的错误。
...
`.connect` 方法如果传入的 URL 路径不以 `/rpc` 结尾，则会自动在末尾添加 `/rpc`。因此，如果一个端点的 URL 路径以 `/rpc` 结尾，则可能会出现与表面相反的结果，即不会抛出错误：
...
当在选择数据库之前未选择命名空间时抛出的错误。或者，当在选择数据库后尝试将命名空间设置为未选择状态时也会抛出此错误。
...
当 RPC 响应指示错误时抛出的错误。连接的协议导致的通信或响应主体解码没有问题，但这意味着 SurrealDB 无法处理 RPC 请求。
...
`.code: number`
:   虽然在 SurrealDB 的文档中没有明确说明，但可能类似于 JSON-RPC 的错误代码。
...
此错误表明错误源于 WebSocket 引擎。当在找到识别 RPC 请求的 ID 之前无法解析响应主体时，或当从 WebSocket 对象接收错误事件时，或当断开 WebSocket 连接时使用。
...
:   以下状态代码之一：
    ...
    - `3150`: 表示从 `WebSocket`[^1] 实例接收了 "error" 事件。
    - `3151`: 表示 WebSocket 引擎在注册的 "open" 事件处理程序中发生错误。大多数情况下，在连接状态到连接状态的过渡过程中会发生 `StateTransitionError`。
    - `3152`: 表示 WebSocket 引擎在注册的 "message" 事件处理程序中发生错误。有多种原因，但最有可能的是在事件处理程序中抛出了 `ResponseError` 或 `RpcResponseError`。追溯到根源，可能是因为在 `.query()` 方法中请求了语法错误的 SurrealQL 自定义查询，或者使用无效的 RPC 参数进行了请求。在这种情况下，RPC 调用方的调用方法 (默认情况下) 会在 5 秒后超时并失败。
    - `3153`: 表示 ping 发送或接收失败。这可能是暂时的错误，但如果持续收到此错误，则可能无法维持连接。
    - `3154`: 表示 WebSocket 引擎在注册的 "close" 事件处理程序中发生错误。大多数情况下，在断开连接状态到断开连接状态的过渡过程中会发生 `StateTransitionError`。
:   请注意，以下状态代码不包含在以上代码中。此 SDK 不将它们视为错误。
    - `1000`: 正常断开连接。
    - `1001`: 早期断开连接很常见。
    - `1004`: 预留。
    - `1005`: 预留。
...
^1: `WebSocket` 可以是运行时环境在全局变量中定义的类，也可以是 `ws` SDK 的类。

## StateTransitionError

继承自：`SurrealAggregateError`

当在状态转换过程中事件监听器执行失败时抛出的错误。
...
`.fallback: number`
:   状态转换失败时的备用转换目标。如果与 `.to` 相同，则表示强制转换。
...
当无法将响应解析为 PRC 响应时抛出的错误。与 `RpcResponseError` 不同。
...
此类是 `@tai-kun/surrealdb/cbor` 明确抛出的所有与 CBOR 相关的错误的基类。它不会直接抛出。
...
此类是 `@tai-kun/surrealdb/cbor` 明确抛出的所有与 CBOR 解码相关的错误的基类。它不会直接抛出。

请参考 RFC8949 的附录 F 中的“ [Well-Formedness Errors and Examples](https://datatracker.ietf.org/doc/html/rfc8949#name-well-formedness-errors-and-)”。
...
当在 CBOR 编码或解码时，JavaScript 对象的深度达到最大值时抛出的错误。每次进入对象或数组时，深度都会增加 1。
