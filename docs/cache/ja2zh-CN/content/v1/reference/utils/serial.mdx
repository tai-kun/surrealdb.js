...
`Serial` 是一个用于生成递增 ID 的类。它用于生成双向通信中 RPC 请求的 ID。ID 是循环生成的，以确保 ID 不会超过安全整数的值。如果 ID 超过该值，它将重置为 1。
...
返回 1 到 `max` 之间的整数。如果此值是最大值，则下一个获取的值将为 1。
