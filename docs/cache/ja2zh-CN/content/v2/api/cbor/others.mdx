...
`write` 是一个辅助函数，可以将所有可以编码为 CBOR 格式的 JavaScript 值写入 `Writer`。
...
如果值不是原始值，并且 `toCBOR` 属性是函数，则优先调用该函数。
...
:   编码器默认会将大多数值编码为 CBOR，但对于那些不符合的特殊值，可以使用可编码的值进行替换。此选项可以通过指定函数或函数数组来实现替换过程。如果函数返回 `Symbol.for("@tai-kun/surrealdb/cbor/continue")` 获取的 `CONTINUE` 符号，则会调用后续函数。如果无法进行任何替换，则会抛出 `SurrealTypeError`。

`isSafeMapKey?: (key: unknown, map: ReadonlyMap<unknown, unknown>) => boolean`
:   此选项是一个函数，用于验证 `Map` 对象的键是否安全。该函数接受两个参数：`key` 和 `map`。`key` 是键的值，`map` 是正在验证的 `Map` 对象。如果此函数返回 `false`，则会抛出 `CborUnsafeMapKeyError`。默认情况下，它会将 `"__proto__"` 或 `"constructor"` 判断为不安全的键。请注意，指定此选项将完全覆盖默认验证。如果您想在默认验证的基础上添加自己的验证，则还应将 `"__proto__"` 或 `"constructor"` 判断为不安全的键。

`isSafeObjectKey?: (key: unknown, obj: { readonly [p: string]: unknown }) => boolean`
:   此选项是一个函数，用于验证普通对象的键是否安全。该函数接受两个参数：`key` 和 `obj`。`key` 是键的值，`obj` 是正在验证的普通对象。如果此函数返回 `false`，则会抛出 `CborUnsafeMapKeyError`。默认情况下，它会将 `"__proto__"` 或 `"constructor"` 判断为不安全的键。请注意，指定此选项将完全覆盖默认验证。如果您想在默认验证的基础上添加自己的验证，则还应将 `"__proto__"` 或 `"constructor"` 判断为不安全的键。
...
数字。如果是整数，则有效的范围是 -2^53-1 到 2^53-1。在 JavaScript 中，没有办法判断 `.0` 是浮点数，因此在这种情况下，它将作为整数编码为 CBOR。例如，`-1.0` 是 `-1`，是一个整数。
...
`writePayload` 是一个辅助函数，可以将 CBOR 数据项的有效载荷写入 `Writer`。
