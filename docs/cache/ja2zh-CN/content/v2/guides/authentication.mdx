...
SurrealDB 提供两种认证方式：用户名和密码认证以及自定义认证。用户名和密码认证需要使用 [`DEFINE USER`](https://surrealdb.com/docs/surrealdb/surrealql/statements/define/user) 语句定义的用户名和密码。自定义认证使用 [`DEFINE ACCESS`](https://surrealdb.com/docs/surrealdb/surrealql/statements/define/access) 语句定义。本文将前者称为“用户认证”，后者称为“访问认证”。
...
以下示例使用记录访问登录 SurrealDB。`.user` 设置为记录 ID：
