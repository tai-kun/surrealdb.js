...
SurrealDB 提供两种身份验证方式：用户名和密码验证以及自定义访问验证。用户名和密码验证需要使用 [`DEFINE USER`](https://surrealdb.com/docs/surrealdb/surrealql/statements/define/user) 语句定义的用户。自定义访问验证则使用 [`DEFINE ACCESS`](https://surrealdb.com/docs/surrealdb/surrealql/statements/define/access) 语句定义。本文将前者称为“用户验证”，后者称为“访问验证”。

### `.signin()` <Badge variant="instance" /><Badge variant="method" />

使用用户验证或访问验证登录 SurrealDB。
...
包含用户验证或访问验证所需信息的认证对象。不同验证类型所需的认证对象参数如下所示：
...
以下示例使用记录访问登录 SurrealDB。`.user` 属性中存储的是记录 ID：
