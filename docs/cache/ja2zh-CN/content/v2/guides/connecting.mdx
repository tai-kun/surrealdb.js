...
通过 `new` 调用时，`Surreal` 会返回该实例。

### `.connect()` <Badge variant="instance" /><Badge variant="method" />

连接到 SurrealDB 终结点。此方法是异步的，但如果在同一实例内，它不会与其他 `.connect()` 或 `.close()` 重复执行。
...
以 URL 形式指定 SurrealDB 的终结点。如果 URL 路径的末尾不是 `/rpc`，则会自动添加。因此，如果将 `http://localhost:8000` 作为参数传递，它将连接到 `http://localhost:8000/rpc`。
...
在没有 `.close()` 断开连接的情况下，无法连接到不同的终结点。此外，先前建立的连接将保持连接：
...
`.connect()` 不会在异步处理的上下文中同时执行，因此以下示例也会导致连接失败。通常情况下，无法确定哪个终结点首先建立连接，因此错误消息可能会有所不同：
...
断开与 SurrealDB 的连接。如果尚未连接或已断开连接，则不会抛出错误。此方法是异步的，但如果在同一实例内，它不会与 `.connect()` 或其他 `.close()` 重复执行。
...
`force?: boolean`
:   如果为 `true`，则会向 `Surreal` 对象的事件监听器发送中断信号。期望事件监听器立即中断，即使处理不完整。
...
获取连接信息。连接信息始终包含内部信息的副本。因此，连接信息中包含 `URL` 对象，但直接修改它不会更改 `Surreal` 对象内部持有的终结点信息。如果尚未请求连接，则为 `undefined`。
...
如果建立了连接，则返回连接信息。否则返回 `undefined`。连接信息包含以下项：
...
为了方便起见，即使尚未建立连接，它也将为 `"closed"`（而不是 `undefined`）。

相关：另请参见 [`.getConnectionInfo()`](#getconnectioninfo)。

### `.endpoint` <Badge variant="instance" /><Badge variant="property" /><Badge variant="readonly" />

连接的终结点。仅在断开连接状态下为 `null`。如果尚未建立连接，则为 `undefined`。
...
`"connecting" | "open" | "closing" | "closed"`
:   连接信息发生变化时或发生变化失败时触发的事件。

`rpc_${RpcMethod}_${number}`
:   双向通信中发生 RPC 响应时触发的事件。`RpcMethod` 例如 `ping` 或 `use` 等 PRC 方法名称。`number` 是双向通信中用于关联请求/响应的标识符，取值为 1 到 2^53-1 之间的整数。在 HTTP 中，它固定为 0。

`live_${string}`
:   接收实时查询事件时触发的事件。目前仅在 WebSocket 协议中使用，在 HTTP 协议中不使用此事件。`string` 是实时查询的 UUID。
...
发生事件时执行的函数。另请参见 [任务发射器](/v2/api/utils/task-emitter#on)。`eventArgs` 中传递的值取决于事件。
...
任务监听器的选项。另请参见 [任务发射器](/v2/api/utils/task-emitter#once)。

#### 返回值

返回一个以 `.on()` 的任务监听器 `eventArgs` 解析的 `StatefulPromise`。
