...
一些错误对象可能包含 `.cause` 属性。在大多数情况下，该属性的类型为 `unknown`，其值可能会在未来版本中发生变化，恕不另行通知。

## 通用错误

### SurrealError

`SurrealError` 是该 SDK 明确抛出的所有错误对象的基类。该类继承自 JavaScript 的 `Error` 类。该错误对象很少单独抛出，通常是其子类被抛出，这些子类具有特定的含义。建议使用 `try-catch` 语句捕获该错误对象，以便轻松地识别它是否是由该 SDK 明确抛出的错误。
...
当检测到循环引用时，会抛出此错误。主要发生在将 JavaScript 值转换为其他格式的过程中，当检测到父对象与自身相同对象时，会抛出此错误。
...
此错误除了在 `toSurql` 函数中以外，也可能会在其他地方抛出。需要仔细调试对象内部，确保对象内部不包含自身。
...
当到达不可到达的代码时，会抛出此错误。如果抛出此错误，可能意味着遇到了该 SDK 的错误。
...
在上述示例中，虽然 https 协议引擎被设置为使用 http 协议引擎，但 http 协议引擎却试图使用 https 协议引擎，从而导致循环引用错误。

#### 解决方法

不要将字符串形式的协议名称设置为另一个协议名称的值，而是用具体的实现来替换创建引擎的过程：
...
检查 `engines` 属性，以确保为目标端点的 URL 的协议创建了引擎。
...
`.connect` 方法会在传入的 URL 路径末尾没有 `/rpc` 时自动添加它。因此，如果一个端点的 URL 路径末尾是 `/rpc`，则可能不会抛出错误，尽管看起来应该抛出错误：
...
在调用 `.connect` 方法之前，可以调用 `.close` 方法来连接到任意端点。
...
当在选择数据库之前没有选择命名空间时，会抛出此错误。或者，当选择数据库后尝试将命名空间设置为未选择状态时，也会抛出此错误。
...
当 RPC 响应表明出现错误时，会抛出此错误。这意味着连接的协议以及响应主体的解码都没有问题，但 SurrealDB 无法处理 RPC 请求。
...
`.code: number`
:   在 SurrealDB 文档中没有明确说明，但可能是 JSON-RPC 错误代码。

#### 解决方法

该错误可能由多种原因导致，但可能是该 SDK 支持的 SurrealDB 版本与处理 RPC 请求的 SurrealDB 版本不一致导致的。
...
表示错误是由 HTTP 引擎引起的。目前只定义了该错误，尚未使用。
...
表示错误是由 WebSocket 引擎引起的。它在解析响应主体失败（在找到标识 RPC 请求的 ID 之前）、从 WebSocket 对象接收错误事件或断开 WebSocket 连接时使用。

该 SDK 使用 `315x` 作为自定义状态码。该范围在 [IANA](https://www.iana.org/assignments/websocket/websocket.xml#close-code-number) 中是未分配的状态码。
...
:   以下状态码之一：
    ...
    - `3151`: 表示 WebSocket 引擎注册的 "open" 事件处理程序中发生错误。通常是由于在连接中状态到连接状态的转换过程中发生了 `StateTransitionError`。
    - `3152`: 表示 WebSocket 引擎注册的 "message" 事件处理程序中发生错误。可能由多种原因导致，但最可能是事件处理程序中抛出了 `ServerResponseError` 或 `RpcResponseError`。这可能是由于使用无效的 RPC 参数发出请求导致的。在这种情况下，RPC 调用方法会（默认情况下）在 5 秒后超时并失败。在 SurrealDB v2.0.2 之前的版本中，超时可能由更广泛的原因导致。
    - `3153`: 表示 ping 的发送或接收失败。这可能是暂时的错误，但如果持续收到此错误，则可能表示连接无法保持。
    - `3154`: 表示 WebSocket 引擎注册的 "close" 事件处理程序中发生错误。通常是由于在断开连接状态到断开连接状态的转换过程中发生了 `StateTransitionError`。
:   注意，不包括以下状态码。该 SDK 不将其视为错误。
    - `1000`: 已正常断开连接。
    - `1001`: 早期断开连接很常见。
    - `1004`: 保留。
    - `1005`: 保留。
...
:   可能没有自动恢复这些状态码的方法。需要修改运行时环境、SurrealDB 的设置或硬编码的实现。
...
`.fallback: string`
:   如果状态转换失败，则回退到的替代转换状态。如果 `.to` 与该值相同，则表示已强制执行转换。
...
当无法将响应解析为 PRC 响应时，会抛出此错误。它与 `RpcResponseError` 不同。
...
通常情况下不会抛出此错误，但如果抛出，则可能是该 SDK 支持的 SurrealDB 版本与处理 RPC 请求的 SurrealDB 版本不一致导致的。
...
它是 `@tai-kun/surrealdb/cbor` 明确抛出的所有与 CBOR 相关的错误的基类。它不会直接抛出。
...
它是 `@tai-kun/surrealdb/cbor` 明确抛出的所有与 CBOR 解码相关的错误的基类。它不会直接抛出。
...
当在编码或解码 CBOR 时，JavaScript 对象的深度达到最大值时，会抛出此错误。进入对象或数组时，深度会增加 1。
...
放宽可选的 `.maxDepth` 上限，或重新设计数据结构，使其嵌套较浅。
...
当在编码或解码 CBOR 时，遇到不安全的映射键时，会抛出此错误。
...
它是 `@tai-kun/surrealdb/json` 明确抛出的所有与 JSON 格式化相关的错误的基类。
