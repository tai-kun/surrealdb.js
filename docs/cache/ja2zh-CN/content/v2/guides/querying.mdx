...
返回一个 `Promise` 对象，它将解析为 SurrealQL 执行结果的数组。如果一个或多个语句失败，则会拒绝带有 `QueryFailedError`，其中包含错误消息。
...
以下示例中，一个语句失败，导致查询被拒绝，并抛出 `QueryFailedError`：
...
在执行 SurrealQL 时传递的变量。请注意，变量名不要以 `
` 开头。
...
返回一个 `Promise` 对象，它将解析为 SurrealQL 执行结果的数组。
