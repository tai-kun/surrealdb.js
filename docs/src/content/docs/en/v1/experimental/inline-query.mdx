...
Inline queries are functions that allow you to execute one-time custom queries without creating an instance of the `Surreal` class. They function very similarly to the [`.query()`](/en/guides/querying#query) method, but it's important to note that they use the `JsonFormatter` by default.
