...
Returns a `Promise` that resolves to an array of the SurrealQL execution results.  If one or more statements fail, it rejects with a `QueryFailedError` containing the error messages.
...
Returns a `Promise` that resolves to an array of the SurrealQL execution results.
