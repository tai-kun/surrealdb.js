...
The `mutex` decorator ensures that one or more class methods are executed asynchronously with a concurrency of 1.

<Aside type="caution">
  While using the pre-built JavaScript SDK is unaffected, direct use of this SDK's source code (TypeScript) requires Stage 3 decorator support.
</Aside>
