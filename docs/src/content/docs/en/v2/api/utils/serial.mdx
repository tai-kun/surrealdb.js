...
The `Serial` class generates incremental IDs. It's used to generate IDs for RPC requests in two-way communication. IDs are generated cyclically to prevent exceeding a safe integer value. If the ID exceeds this value, it resets to 1.
