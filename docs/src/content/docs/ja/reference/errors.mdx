...
これは到達不可能なコードに到達した場合に投げられるエラーです。このエラーが投げられた場合、おそらくこのパッケージのバグに遭遇したものと考えられます。
...
これはクライアントが同時に異なる複数の名前空間に切り替えようとした場合に投げられるエラーです。
...
これはクライアントが同時に異なる複数のデータベースに切り替えようとした場合に投げられるエラーです。
...
await db.query("OUTPUT 'Hello'"); // throws QueryFailure: Query failed with 1 error(s)
```
...
:   このエラーが致命的であるかどうかを示しています。ただしこれが `true` であっても切断する必要がないことがあります。
...
:   次のステータスコードのうちのいずれか一つです:
    ...
    - `3152`: WebSocket エンジンが登録する "message" イベントハンドラー内でエラーが発生したことを示しています。多用な原因が考えられますが、イベントハンドラー内で `ResponseError` が投げられた可能性が高いです。
...
:   なお、上記から次のステータスコードを除きます。このパッケージではエラーとして扱いません。
    - `1000`: 正常に切断されています。
    - `1001`: 早期の切断はよくあります。
    - `1004`: 予約済み。
    - `1005`: 予約済み。
...
^1: `WebSocket` はランタイムがグローバル変数に定義しているクラスであったり、`ws` パッケージのクラスであったりします。
