---
title: インラインクエリー
slug: ja/v1/experimental/inline-query
banner:
  content: これは実験的な機能であり、API は頻繁に変更される可能性があります。
---

インラインクエリーは `Surreal` クラスのインスタンスを作成しなくとも、その場で 1 回限りのカスタムクエリーを実行する関数です。使い方は [`.query()`](/ja/guides/querying#query) メソッドとほとんど同じです。ただしデフォルトでは `JsonFormatter` が使用されていることに注意してください。
