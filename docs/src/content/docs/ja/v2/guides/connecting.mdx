...
`rpc_${RpcMethod}_${number}`
:   双方向通信における RPC レスポンスが発生するイベントです。`RpcMethod` は例えば `ping` や `use` などの PRC メソッド名です。`number` は双方向通信においてリクエスト/レスポンスを紐付けるための識別子であり、1 から 2^53-1 までの整数をとります。HTTP では 0 で固定になります。
