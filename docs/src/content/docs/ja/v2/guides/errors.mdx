...
:   次のステータスコードのうちのいずれか一つです:
    ...
    - `3152`: WebSocket エンジンが登録する "message" イベントハンドラー内でエラーが発生したことを示しています。多用な原因が考えられますが、イベントハンドラー内で `ResponseError` または `RpcResponseError` が投げられた可能性が高いです。無効な RPC パラメーターでリクエストしたことが原因と考えられます。その場合、RPC の呼び出し元のメソッドは (デフォルトで) 5 秒でタイムアウトして失敗します。SurrealDB v2.0.2 より前のバージョンでは、より広範な原因でタイムアウトする可能性があります。
