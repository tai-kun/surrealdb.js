...
`initialDelay?: number`
:   처음 재연결을 시도할 때까지의 지연 시간(밀리초)입니다. 기본값은 `500`입니다.

`maxDelay?: number`
:   재연결 지연 시간(밀리초)의 최대값입니다. 백오프가 진행될수록 지연 시간이 증가하지만 이 값이 상한선이 됩니다. 기본값은 `30_000`(30초)입니다.
...
:   현재 재연결 정보(`.state` 및 `.phase`)를 가져옵니다. 각각 다음 의미를 나타냅니다:
:   `.state`:
...
:   `.phase`:
    - `"waiting"`       ... 초기 상태입니다. 재연결은 한 번도 수행되지 않았습니다.
    - `"pending"`       ... 재연결이 요청되었으며, 실행될 때까지 일정 시간 대기 중입니다.
...
`.enabled: boolean`
:   재연결 기능이 활성화되었는지 여부를 나타내는 불리언 값입니다. `true`이면 재연결이 활성화되어 있습니다.
...
- 재연결 로직은 WebSocket 연결의 안정성과 연결 복구 빈도를 적절히 조정하는 데 중요합니다. 옵션 설정은 시스템 요구 사항 및 사용 사례에 따라 조정해야 합니다.
- `autoReconnect`는 연결이 끊어졌을 때 복구를 시도하는 것이며, 모든 경우에 완벽한 복구가 보장되는 것은 아닙니다.
