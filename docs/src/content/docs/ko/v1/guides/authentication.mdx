...
SurrealDB에 인증하는 방법은 두 가지가 있습니다. 첫 번째는 사용자 이름과 비밀번호를 사용하는 방법입니다. 이 방법은 [`DEFINE USER`](https://surrealdb.com/docs/surrealdb/surrealql/statements/define/user) 문을 사용하여 정의된 사용자 이름과 비밀번호가 필요합니다. 두 번째는 [`DEFINE ACCESS`](https://surrealdb.com/docs/surrealdb/surrealql/statements/define/access) 문을 사용하여 정의된 사용자 지정 인증입니다. 이 문서에서는 전자를 "사용자 인증", 후자를 "액세스 인증"이라고 부릅니다.

### `.signin()` <Badge variant="instance" /><Badge variant="method" />

SurrealDB에 사용자 인증 또는 액세스 인증으로 로그인합니다.
...
다음 예제는 루트 사용자로 SurrealDB에 로그인합니다:
...
다음 예제는 네임스페이스 사용자로 SurrealDB에 로그인합니다:
...
다음 예제는 데이터베이스 사용자로 SurrealDB에 로그인합니다:
...
다음 예제는 범위 사용자로 SurrealDB에 로그인합니다. `.user`에는 레코드 ID가 설정됩니다:
