...
`new`를 통해 호출되면 `Surreal`은 해당 인스턴스를 반환합니다.
...
SurrealDB 엔드포인트를 URL로 지정합니다. URL 경로의 끝이 `/rpc`가 아니면 자동으로 추가됩니다. 따라서 `http://localhost:8000`를 인수로 전달하면 `http://localhost:8000/rpc`에 연결됩니다.
...
다음 예에서는 WebSocket 프로토콜을 통해 SurrealDB에 연결합니다.
...
`.disconnect()`로 연결을 끊지 않고 다른 엔드포인트에 연결할 수 없습니다. 이전에 연결이 설정된 엔드포인트와의 연결은 유지됩니다.
...
`.connect()`는 비동기 처리 컨텍스트에서 동시에 실행될 수 없으므로 다음 예제에서도 연결이 실패합니다. 일반적으로 어떤 엔드포인트가 먼저 연결을 설정할지 알 수 없으므로 오류 메시지는 다를 수 있습니다.
...
SurrealDB와의 연결을 끊습니다. 아직 연결되지 않았거나 이미 연결이 끊어진 경우에도 오류를 발생시키지 않습니다. 이 메서드는 비동기적으로 실행되지만, 동일한 인스턴스 내에서 `.connect()` 또는 다른 `.disconnect()`와 중복되어 실행되지 않습니다.
...
다음 예에서는 WebSocket 프로토콜을 통해 SurrealDB에 연결한 다음 연결을 끊습니다.
...
연결 정보를 가져옵니다. 연결 정보는 항상 내부 정보를 복사한 값을 가지고 있습니다. 따라서 연결 정보에는 `URL` 객체가 포함되어 있지만 이를 직접 수정해도 `Surreal` 객체가 내부적으로 가지고 있는 엔드포인트 정보는 변경되지 않습니다. 아직 연결 요청이 없으면 `undefined`입니다.
...
`rpc_${RpcMethod}_${number}`
:   양방향 통신에서 RPC 응답이 발생하는 이벤트입니다. 현재 WebSocket 프로토콜에서만 사용되며 HTTP 프로토콜에서는 이 이벤트가 사용되지 않습니다. `RpcMethod`는 예를 들어 `ping` 또는 `use`와 같은 PRC 메서드 이름입니다. `number`는 양방향 통신에서 요청/응답을 연결하기 위한 식별자이며 1부터 2^53-1까지의 정수를 취합니다.
...
태스크 리스너 옵션입니다. [태스크 에미터](/ko/reference/utils/task-emitter#once)를 참조하십시오.
