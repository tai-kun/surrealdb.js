...
`new`를 통해 호출된 경우, `Surreal`은 해당 인스턴스를 반환합니다.
...
`force?: boolean`
:   `true`로 설정하면 `Surreal` 객체의 이벤트 리스너에 전달되는 취소 신호에 취소 신호를 전송합니다. 이벤트 리스너는 처리가 불완전하더라도 즉시 중단될 것으로 예상됩니다.
...
연결 정보를 가져옵니다. 연결 정보는 항상 내부 정보가 복사된 값을 가지고 있습니다. 따라서 연결 정보에는 `URL` 객체가 포함되지만, 이를 직접 변경해도 `Surreal` 객체가 내부적으로 가지고 있는 엔드포인트 정보는 변경되지 않습니다. 아직 연결 요청이 되지 않은 경우 `undefined`입니다.
...
연결이 설정되어 있는 경우 연결 정보를 반환합니다. 그렇지 않은 경우 `undefined`를 반환합니다. 연결 정보에는 다음 항목이 포함됩니다.
...
편의상, 연결이 설정되지 않은 경우에도 `3`이 됩니다 (`undefined`가 아닙니다).
...
현재 인증된 토큰입니다. 회원가입 또는 로그인하지 않은 경우 `null`입니다. 연결이 설정되지 않은 경우 `undefined`입니다.
...
`rpc_${RpcMethod}_${number}`
:   양방향 통신에서 RPC 응답이 발생할 때 발생하는 이벤트입니다. 현재 WebSocket 프로토콜에서만 사용되며, HTTP 프로토콜에서는 이 이벤트가 사용되지 않습니다. `RpcMethod`는 예를 들어 `ping`이나 `use`와 같은 PRC 메서드 이름입니다. `number`는 양방향 통신에서 요청/응답을 연결하기 위한 식별자이며, 1부터 2^53-1까지의 정수를 취합니다.
