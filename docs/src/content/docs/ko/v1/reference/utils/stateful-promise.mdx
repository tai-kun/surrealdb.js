...
다음 예에서는 `StatefulPromise` 가 해결될 때까지 기다리고, 해당 결과를 받습니다:
...
다음 예에서는 `StatefulPromise` 가 거부될 때까지 기다리고, 처리하면 오류가 발생할 것으로 예상합니다:
...
일반 `Promise` 는 처리되지 않은 상태로 거부될 경우 (즉, `reject` 가 호출될 경우) `unhandledrejection` 이 발생하지만, `StatefulPromise` 는 명시적으로 처리할 때까지 오류를 던지지 않는다는 것을 알 수 있습니다.
...
인수의 `value` 로 해결된 `StatefulPromise` 객체를 반환합니다. 값이 `StatefulPromise` 객체라면, 해당 값을 반환합니다.
...
`.withResolvers` 는 새로운 `StatefulPromise` 객체와, 이를 해결하거나 거부하는 함수를 포함하는 객체를 반환하는 함수입니다.
...
호출될 함수입니다. 이 함수는 `StatefulPromise` 가 아닌 값을 반환할 수 있습니다. 또한 함수 내에서 동기적으로 오류를 던질 수도 있습니다.
...
다음 예는 동기적으로 던져진 오류가 `StatefulPromise` 에 의해 포착되는 것을 보여줍니다:
...
반복 가능한 객체입니다. `StatefulPromise` 객체 이외의 것을 포함할 수도 있지만, 이들은 오류로 간주되지 않습니다. 또한 일반 `Promise` 도 마찬가지로 무시됩니다.
