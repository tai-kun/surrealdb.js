...
`Datetime`은 날짜 및 시간을 표현하는 클래스입니다. SurrealQL의 [`datetime`](https://surrealdb.com/docs/surrealql/datamodel/datetimes) 형식과 호환됩니다.
...
초 단위와 나노초 단위의 쌍 또는 문자열입니다. standard 이하 프리셋의 경우 `Date` 객체 또는 `bigint` 입력도 지원합니다.

##### `year`, `monthIndex`, ...

`Date` 클래스의 인수와 동일합니다. 단, `nanoseconds` 입력도 지원합니다. `undefined`를 제공한 인수 뒤에 `number`를 제공할 수 없습니다. 이는 `Date` 클래스와 거의 동일한 사양입니다.

#### 반환 값

`new`를 통해 호출되면 `Datetime`은 해당 인스턴스를 반환합니다. standard 프리셋의 경우 `Date` 인스턴스이기도 합니다.

### `.seconds` <Badge variant="instance" /><Badge variant="property" /><Badge variant="decodeonly" />

나노초 정밀도 시간의 초 부분입니다. standard 이하 프리셋에서는 읽기 전용입니다. 값은 -2<sup>53</sup>-1에서 2<sup>53</sup>-1까지의 정수 또는 `NaN`입니다.
...
나노초 정밀도 시간의 나노초 부분입니다. standard 이하 프리셋에서는 읽기 전용입니다. 값은 0에서 999999999까지의 정수 또는 `NaN`입니다.
...
나노초 정밀도 시간의 문자열을 ISO 형식으로 가져옵니다. `Date` 객체의 `.toISOString()`과 유사하지만, 나노초 정밀도이므로 소수점 아홉 번째 자리까지 문자열화됩니다.
...
`Datetime` 객체를 SurrealQL에 삽입할 수 있는 문자열로 변환합니다. `.toISOString()`과 유사하지만, `d` 접두어를 추가하여 쿼리 파서에 문자열이 UUID임을 알립니다.
...
`d` 접두어가 붙은 ISO 형식의 날짜 및 시간을 반환합니다.
...
`.getUTCMicroseconds()`는 이 메서드와 동일하게 동작합니다.
...
`.setUTCMicroseconds()`는 이 메서드와 동일하게 동작합니다.
...
설정할 마이크로초입니다. 값은 정수로 변환됩니다. 값이 0에서 999 사이가 아니면 전체 시간이 영향을 받는다는 점에 유의하십시오.
...
`.getUTCNanoseconds()`는 이 메서드와 동일하게 동작합니다.
...
`.setUTCNanoseconds()`는 이 메서드와 동일하게 동작합니다.
...
설정할 나노초입니다. 값은 정수로 변환됩니다. 값이 0에서 999 사이가 아니면 전체 시간이 영향을 받는다는 점에 유의하십시오.
...
#### `Datetime` 객체의 일반적인 판단

프리셋에 의존하지 않고 값이 `Datetime` 객체인지 판단하려면 `isDatetime` 함수를 사용하는 것이 좋습니다. 이 함수는 `instanceof`를 사용하지 않으므로 검증할 값의 프리셋이 알 수 없는 경우에 유용합니다.
...
SurrealDB의 `datetime` 형식은 나노초 정밀도까지 유지할 수 있습니다. 따라서 JavaScript로 처리하는 데는 주의가 필요합니다. SurrealDB가 직렬화한 `datetime` (예: `"1969-12-31T23:59:59.999999999Z"`)을 JavaScript의 `Date.parse`에 전달하면 WebKit에서는 0밀리초가 되지만, 그 밖의 (Node.js, Deno, Bun, Chromium, Firefox)에서는 -1밀리초가 됩니다.
