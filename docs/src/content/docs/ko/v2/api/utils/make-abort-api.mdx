...
취소 신호입니다. 초기 상태에서 취소되지 않습니다.

##### `abort`

취소 신호에 `"abort"` 이벤트를 발송합니다. 옵션으로 하나의 인수 `reason`을 받아 `"abort"` 이벤트를 통해 취소 사유를 전달할 수 있습니다.

#### 예

다음 예제에서는 인수 없이 Abort API를 만듭니다.
...
위의 예제는 `new AbortController()`와 거의 동일한 의미를 갖습니다. `AbortController`와 다른 점은 `makeAbortApi`가 객체가 아니라 배열을 반환한다는 것입니다.

다음 예제에서는 연결할 취소 신호를 지정하여 Abort API를 만듭니다.
