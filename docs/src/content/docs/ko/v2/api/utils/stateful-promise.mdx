...
다음 예에서는 `StatefulPromise`가 거부될 때까지 대기하고 처리하면 오류를 던질 것으로 예상합니다.
...
일반 `Promise`는 처리되지 않은 상태로 거부되는 경우(즉, `reject`가 호출되는 경우) `unhandledrejection`이 발생하지만, `StatefulPromise`는 명시적으로 처리될 때까지 오류를 던지지 않는다는 것을 알 수 있습니다.
...
`.then`은 `StatefulPromise`가 성공하거나 실패하는 경우의 콜백 함수를 가져와 `StatefulPromise` 객체를 반환합니다.
...
`.withResolvers`는 새 `StatefulPromise` 객체와 해당 객체를 해결하거나 거부하는 함수가 포함된 객체를 반환하는 함수입니다.
...
호출할 함수입니다. 이 함수는 `StatefulPromise` 이외의 값을 반환할 수 있습니다. 또한 함수 내에서 동기적으로 오류를 던질 수도 있습니다.
...
다음 예는 동기적으로 던져진 오류가 `StatefulPromise`에 의해 처리되는 것을 보여줍니다.
...
반복 가능한 객체입니다. `StatefulPromise` 객체 이외의 객체를 포함할 수도 있지만, 해당 객체는 오류로 간주되지 않습니다. 일반 `Promise`도 마찬가지로 무시됩니다.
