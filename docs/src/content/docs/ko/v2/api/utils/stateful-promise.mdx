...
`new`를 통해 호출된 경우, `StatefulPromise`는 자신의 인스턴스를 반환합니다.

#### 예

##### 해결

다음 예제에서는 `StatefulPromise`가 해결될 때까지 대기하고 결과를 받습니다:
...
다음 예제에서는 `StatefulPromise`가 거부될 때까지 대기하고 처리하면 오류가 발생할 것으로 예상됩니다:
...
일반 `Promise`의 경우 처리되지 않은 상태로 거부되면(즉, `reject`가 호출되면) `unhandledrejection`이 발생하지만, `StatefulPromise`는 명시적으로 처리할 때까지 오류를 던지지 않습니다.
...
`.withResolvers`는 새로운 `StatefulPromise` 객체와 해당 객체를 해결하거나 거부하는 함수를 포함하는 객체를 반환하는 함수입니다.
...
호출될 함수입니다. 이 함수는 `StatefulPromise` 이외의 값을 반환할 수 있습니다. 또한 함수 내에서 동기적으로 오류를 던질 수 있습니다.

#### 반환 값

- `func`가 동기적으로 값을 반환하는 경우, 해결된 상태의 `StatefulPromise`를 반환합니다.
- `func`가 동기적으로 오류를 던지는 경우, 거부된 상태의 `StatefulPromise`를 반환합니다.
- `func`가 비동기적으로 해결하거나 거부하는 경우, 보류 상태의 `StatefulPromise`를 반환합니다.
...
반복 가능한 객체입니다. `StatefulPromise` 객체 이외의 것을 포함할 수도 있지만, 오류로 간주되지 않습니다. 또한 일반 `Promise`도 마찬가지로 무시됩니다.
...
`StatefulPromise` 객체가 거부한 이유 배열로 해결된 `StatefulPromise` 객체를 반환합니다.
