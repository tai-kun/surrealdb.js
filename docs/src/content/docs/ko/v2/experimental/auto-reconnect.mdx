...
`maxDelay?: number`
:   재연결 지연 시간(밀리초)의 최댓값입니다. 백오프가 진행됨에 따라 지연 시간이 증가하지만, 이 값이 상한선이 됩니다. 기본값은 `30_000`(30초)입니다.
...
:   `.phase`:
    ...
    - `"closing"`    ... 연결을 끊는 중입니다.
    - `"connecting"` ... 연결 중입니다. 연결 끊김에 실패해도 연결을 시도합니다.
...
`.enabled: boolean`
:   재연결 기능이 활성화되어 있는지 여부를 나타내는 불리언 값입니다. `true`인 경우 재연결이 활성화되어 있습니다.
