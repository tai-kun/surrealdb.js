...
일부 에러 객체에는 `.cause` 프로퍼티가 설정되어 있을 수 있습니다. 대부분의 경우 `unknown` 유형이며, 값은 추후 예고 없이 변경될 수 있습니다.

## 일반

### SurrealError

`SurrealError`는 이 SDK에서 명시적으로 던지는 대부분의 에러 객체가 상속하는 기본 클래스입니다. 이 클래스는 JavaScript의 `Error` 클래스를 상속합니다. 이 에러 객체가 단독으로 던져지는 경우는 거의 없고, 대부분은 상속받은 특별한 의미를 가진 에러 객체가 던져집니다. `try-catch` 구문 등을 통해 잡은 에러가 이 SDK에서 명시적으로 던진 에러인지 여부를 쉽게 분류하기 위해 사용하는 것이 좋습니다.
...
`.cause`
:   에러가 발생한 원인 또는 컨텍스트가 설정될 수 있습니다.

### SurrealTypeError

상속 원본: `SurrealError`

입력 값 또는 다른 검증에 실패한 경우 던져지는 에러입니다.
...
이 에러는 `toSurql` 함수 이외에도 던져지는 일반적인 에러입니다. 객체 내부에 해당 객체가 포함되어 있는지 신중하게 디버깅해야 합니다.
...
데이터베이스에 연결할 때, 엔진 간 순환 참조가 발생한 경우 던져지는 에러입니다.
...
`.seen: string[]`
:   순환 참조가 발생할 때까지 거쳐 온 스키마 이름 목록입니다.
...
위 예시에서는 https 프로토콜 엔진이 http에 설정된 엔진을 사용하도록 지정했지만, http 프로토콜 엔진은 https에 설정된 엔진을 사용하려고 하기 때문에 순환 참조 에러가 던져집니다.
...
연결할 엔드포인트의 프로토콜로 엔진을 생성할 수 있는지 `engines` 프로퍼티를 확인합니다.
...
클라이언트가 여러 개의 엔드포인트에 동시에 연결하려고 할 때 던져지는 에러입니다.
...
`.connect` 메서드는 전달된 URL의 경로가 `/rpc`로 끝나지 않으면, `/rpc`를 뒤에 추가합니다. 따라서, 한쪽 엔드포인트의 URL 경로가 `/rpc`로 끝나는 경우, 외관상으로는 에러가 발생하지 않을 수 있습니다.
...
데이터베이스를 선택하기 전에 네임스페이스가 선택되지 않은 경우 던져지는 에러입니다. 또는, 데이터베이스를 선택한 상태로 네임스페이스를 선택하지 않은 상태로 만들려고 할 때도 던져집니다.
...
RPC 응답이 에러를 나타내는 경우 던져지는 에러입니다. 연결된 프로토콜을 통한 통신이나 응답 본문 디코딩 등에 문제가 없지만, SurrealDB가 RPC 요청을 처리할 수 없음을 의미합니다.
...
`.code: number`
:   SurrealDB 문서에는 명시되어 있지 않지만, 아마도 JSON-RPC 에러 코드일 것입니다.

#### 해결책

다양한 원인이 있을 수 있지만, 이 SDK가 지원하는 SurrelDB 버전과 RPC 요청을 처리하는 SurrealDB 버전이 다를 가능성이 있습니다.
...
엔진에서 발생한 에러임을 나타냅니다. 이벤트 리스너를 통해 전송됩니다.
...
WebSocket 엔진에서 발생한 에러임을 나타냅니다. RPC 요청을 식별하는 ID를 찾기 전에 응답 본문을 분석하는 데 실패하거나, WebSocket 객체에서 에러 이벤트를 받았을 때, WebSocket 연결을 끊었을 때 등에 사용됩니다.

이 SDK에서는 `315x`를 사용자 정의 상태 코드로 사용합니다. 이 범위는 [IANA](https://www.iana.org/assignments/websocket/websocket.xml#close-code-number)에서 할당되지 않은 상태 코드입니다.
...
:   다음 상태 코드 중 하나입니다:
    ...
    - `3151`: WebSocket 엔진에서 등록한 "open" 이벤트 핸들러 내에서 에러가 발생했음을 나타냅니다. 연결 중 상태에서 연결 상태로 전환되는 중에 `StateTransitionError`가 발생했을 가능성이 높습니다.
    - `3152`: WebSocket 엔진에서 등록한 "message" 이벤트 핸들러 내에서 에러가 발생했음을 나타냅니다. 다양한 원인이 있을 수 있지만, 이벤트 핸들러 내에서 `ServerResponseError` 또는 `RpcResponseError`가 던져졌을 가능성이 높습니다. 잘못된 RPC 매개변수로 요청했을 수 있습니다. 이 경우, RPC 호출 원본 메서드는 (기본적으로) 5초 후에 시간 초과되어 실패합니다. SurrealDB v2.0.2 이전 버전에서는 더 광범위한 원인으로 시간 초과될 수 있습니다.
    - `3153`: ping을 보내거나 받는 데 실패했음을 나타냅니다. 일시적인 에러일 수 있지만, 계속해서 받는다면 연결이 유지되지 않을 수 있습니다.
    - `3154`: WebSocket 엔진에서 등록한 "close" 이벤트 핸들러 내에서 에러가 발생했음을 나타냅니다. 연결 해제 중 상태에서 연결 해제 상태로 전환되는 중에 `StateTransitionError`가 발생했을 가능성이 높습니다.
:   위에서 다음 상태 코드는 제외됩니다. 이 SDK에서는 에러로 처리하지 않습니다.
    - `1000`: 정상적으로 연결이 끊어졌습니다.
    - `1001`: 조기 연결 해제는 흔합니다.
    - `1004`: 예약됨.
    - `1005`: 예약됨.
    - `1006`: 예약됨.
...
:   이 상태 코드에서 자동으로 복구할 수 있는 방법은 아마 없을 것입니다. 런타임이나 SurrealDB 설정, 하드코딩된 구현을 수정해야 합니다.

`3152`
:   메서드에 전달한 인수(즉, RPC 요청의 내용)가 올바른지 확인합니다.

`1012`,`1013`,`3153`
:   실험적인 기능인 [`autoReconnect()`](/v2/experimental/auto-reconnect)를 사용하면 자동으로 복구할 수 있을 수도 있습니다.
...
`.fallback: string`
:   상태 전환에 실패한 경우 대체 전환할 상태입니다. `.to`와 같은 값이면, 강제로 전환했음을 의미합니다.
...
연결되지 않은 상태로 RPC 요청을 보내려고 할 때 던져지는 에러입니다.
...
응답을 PRC 응답으로 분석할 수 없는 경우 던져지는 에러입니다. `RpcResponseError`와는 다릅니다.
...
일반적으로 이 에러가 던져지는 경우는 없어야 하지만, 만약 던져진다면, 이 SDK가 지원하는 SurrelDB 버전과 RPC 요청을 처리하는 SurrealDB 버전이 다를 가능성이 있습니다.
...
`@tai-kun/surrealdb/cbor`에서 명시적으로 던지는, CBOR 관련 모든 에러가 상속하는 클래스입니다. 이 에러는 직접 던져지지 않습니다.
...
`@tai-kun/surrealdb/cbor`에서 명시적으로 던지는, CBOR 디코딩 관련 모든 에러가 상속하는 클래스입니다. 이 에러는 직접 던져지지 않습니다.

RFC8949 부록 F에 있는 "Well-Formedness Errors and Examples"를 참조하십시오.
...
CBOR 인코딩 또는 디코딩 중에 JavaScript 객체의 깊이가 최대값에 도달한 경우 던져지는 에러입니다. 객체 또는 배열에 들어갈 때마다 깊이가 1 증가합니다.
...
CBOR 인코딩 또는 디코딩 중에 안전하지 않은 맵 키가 발견된 경우 던져지는 에러입니다.
...
`@tai-kun/surrealdb/json`에서 명시적으로 던지는, JSON-formatter.
...
JSON 디코딩 중에 안전하지 않은 맵 키가 발견된 경우 던져지는 에러입니다.
