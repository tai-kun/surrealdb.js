...
SurrealQL 실행 결과의 배열로 해결되는 `Promise` 객체를 반환합니다. 하나 이상의 문이 실패한 경우 `QueryFailedError`로 거부합니다. 이 오류는 실패한 문의 오류 메시지를 포함합니다.
...
다음 예에서는 한 쪽 문이 실패하기 때문에 쿼리는 `QueryFailedError`로 거부됩니다.
