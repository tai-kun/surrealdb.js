...
普通对象的判断使用 [`is-plain-obj`](https://github.com/sindresorhus/is-plain-obj)。因此，如果普通对象既不具有 `.toSurql` 也不具有 `.toJSON`，并且具有 `Symbol.toStringTag` 或 `Symbol.iterator`，则不会将其判断为对象，而是抛出错误。
