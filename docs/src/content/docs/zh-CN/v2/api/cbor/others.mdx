...
-  `Uint8Array`
-  `Simple` 对象
-  可以被可选的 `replacer` 替换的值

如果值不是原始值，并且 `toCBOR` 属性是一个函数，则优先调用它。
...
:   编码器默认将大多数值编码为 CBOR，但可以将那些不符合要求的特殊值替换为可编码的值。此选项通过指定函数或函数数组来实现替换过程。如果函数返回 `Symbol.for("@tai-kun/surrealdb/cbor/continue")` 获取的 `CONTINUE` 符号，则会调用后续函数。如果无法替换任何内容，则会抛出 `SurrealTypeError`。

`isSafeMapKey?: (key: unknown, map: ReadonlyMap<unknown, unknown>) => boolean`
:   此选项是一个函数，用于验证 `Map` 对象的键是否安全。该函数接受两个参数：`key` 和 `map`。`key` 是键的值，`map` 是正在验证的 `Map` 对象。如果此函数返回 `false`，则会抛出 `CborUnsafeMapKeyError`。默认情况下，它将 `"__proto__"` 或 `"constructor"` 判定为不安全的键。请注意，如果指定了此选项，它将完全覆盖默认验证。如果要将自己的验证添加到默认验证中，则应将 `"__proto__"` 或 `"constructor"` 判定为不安全的键。

`isSafeObjectKey?: (key: unknown, obj: { readonly [p: string]: unknown }) => boolean`
:   此选项是一个函数，用于验证普通对象的键是否安全。该函数接受两个参数：`key` 和 `obj`。`key` 是键的值，`obj` 是正在验证的普通对象。如果此函数返回 `false`，则会抛出 `CborUnsafeMapKeyError`。默认情况下，它将 `"__proto__"` 或 `"constructor"` 判定为不安全的键。请注意，如果指定了此选项，它将完全覆盖默认验证。如果要将自己的验证添加到默认验证中，则应将 `"__proto__"` 或 `"constructor"` 判定为不安全的键。
...
数字。对于整数，有效值为 -2^53-1 到 2^53-1。在 JavaScript 中，没有方法可以将 `.0` 判定为浮点数，因此它将被编码为 CBOR 中的整数。例如，`-1.0` 是 `-1`，它是整数。
