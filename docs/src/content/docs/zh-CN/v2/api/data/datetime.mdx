...
秒数和纳秒数的数组，或字符串。如果预设为 standard，则还支持 `Date` 对象或 `bigint` 作为输入。

##### `year`, `monthIndex`, ...

与 `Date` 类中的参数相同。但它也支持 `nanoseconds` 的输入。不能在提供 `undefined` 的参数之后再提供 `number`。这与 `Date` 类大体相同。

#### 返回值

如果通过 `new` 调用，`Datetime` 将返回其实例。如果预设为 standard，则它也是 `Date` 的实例。

### `.seconds` <Badge variant="instance" /><Badge variant="property" /><Badge variant="decodeonly" />

纳秒精度时间中的秒部分。在低于 standard 的预设中，它是只读的。值是一个介于 -2<sup>53</sup>-1 到 2<sup>53</sup>-1 之间的整数或 `NaN`。
...
纳秒精度时间中的纳秒部分。在低于 standard 的预设中，它是只读的。值是一个介于 0 到 999999999 之间的整数或 `NaN`。
...
获取毫秒精度时间的字符串表示形式。与 `Date` 对象的 `.toString()` 相同。
...
获取纳秒精度时间的 ISO 格式字符串。与 `Date` 对象的 `.toISOString()` 类似，但由于是纳秒精度，因此字符串化到小数点后第九位。
...
将 `Datetime` 对象转换为可嵌入 SurrealQL 的字符串。与 `.toISOString()` 类似，但它添加了 `d` 前缀，以告知查询解析器该字符串是 UUID。
...
要设置的微秒数。该值将被转换为整数。注意，如果该值不在 0 到 999 之间，则整个时间将会受到影响。
...
要设置的纳秒数。该值将被转换为整数。注意，如果该值不在 0 到 999 之间，则整个时间将会受到影响。
...
建议使用 `isDatetime` 函数来判断值是否是 `Datetime` 对象，而无需依赖预设。此函数不使用 `instanceof`，因此在验证值预设未知的情况下很有用。
...
SurrealDB 的 `datetime` 类型可以保持到纳秒精度。因此，在 JavaScript 中处理时需要注意。如果将 SurrealDB 序列化后的 `datetime`（例如 `"1969-12-31T23:59:59.999999999Z"`）传递给 JavaScript 的 `Date.parse`，则在 WebKit 中它将为 0 毫秒，而在其他环境（Node.js、Deno、Bun、Chromium、Firefox）中它将为 -1 毫秒。
