...
UUID（Universally Unique Identifier）是一个 128 位的标识符，用于唯一标识特定实体。`Uuid` 类用于表示 UUID。它支持 UUID v1 到 v7。需要注意的是，生成 UUID 需要使用额外的库或运行时中内置的功能（例如 `node:crypto`）。
...
UUID 的变体。它由 UUID 第 13 位的最高位定义，有以下 3 种主要变体和 1 种为将来保留的变体。
...
2. RFC 4122 兼容
    - `0b10xx`
    - 基于 RFC 4122（UUID 的标准规范）的 UUID。
    - 是最常用的 UUID 格式，包括 v1（基于时间）和 v4（随机）等格式。
3. Microsoft GUID 兼容
    - `0b110x`
...
此属性除了上述变体外，还表示 NIL UUID 和 MAX UUID。
...
如果 UUID 的变体为 `"RFC"` 且版本为 v1、v6 或 v7 之一，则为时间戳部分。否则为 `null`。为了方便起见，所有时间戳都计算为自 UNIX 纪元以来的毫秒数。
...
将 `Uuid` 对象转换为可以嵌入 SurrealQL 的字符串。与 `.toString()` 类似，但通过添加 `u` 前缀来告诉查询解析器该字符串是一个 UUID。
...
如果任一方没有时间戳部分，或者时间戳相同，则返回 `0`。如果要比较的 UUID 的时间戳更晚，则返回 `-1`，如果更早，则返回 `1`。
...
建议使用 `isUuid` 函数来判断值是否为 `Uuid` 对象，而不依赖于预设。该函数不使用 `instanceof`，因此在验证值预设未知的情况下非常有用。
