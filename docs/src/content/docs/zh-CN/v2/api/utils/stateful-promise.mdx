...
`StatefulPromise` 是一个实现了 `PromiseLike` 接口的类，即使在未处理的情况下被拒绝，它也不会抛出错误。
...
当通过 `new` 调用时，`StatefulPromise` 会返回它的实例。

#### 例子

##### 解决

以下示例展示了如何等待 `StatefulPromise` 解决并接收其结果：
...
可以看出，普通的 `Promise` 在未处理的情况下被拒绝（即调用 `reject`）会导致 `unhandledrejection`，而 `StatefulPromise` 不会在明确处理之前抛出错误。
...
`.then` 接收成功和失败时的回调函数，并返回一个 `StatefulPromise` 对象。
...
返回一个使用参数 `value` 解决的 `StatefulPromise` 对象。如果值本身是 `StatefulPromise` 对象，则返回该值。
...
返回一个使用参数 `reason` 拒绝的 `StatefulPromise`。

### `.withResolvers()` <Badge variant="static" /><Badge variant="method" />

`.withResolvers` 是一个返回新的 `StatefulPromise` 对象以及一个包含解决或拒绝该对象的函数的对象的函数。
...
要调用的函数。该函数可以返回除 `StatefulPromise` 以外的值。它也可以在函数内部同步抛出错误。

#### 返回值

- 如果 `func` 同步返回一个值，则返回一个已解决状态的 `StatefulPromise`。
- 如果 `func` 同步抛出错误，则返回一个已拒绝状态的 `StatefulPromise`。
- 如果 `func` 异步解决或拒绝，则返回一个挂起状态的 `StatefulPromise`。
...
`.allRejected` 是一个仅收集拒绝的 `StatefulPromise` 的原因的函数。
...
可迭代对象。它可以包含非 `StatefulPromise` 对象，但它们不会被视为错误。普通的 `Promise` 同样会被忽略。
...
返回一个已解决的 `StatefulPromise` 对象，其中包含拒绝的 `StatefulPromise` 对象的原因数组。
