...
连接到 SurrealDB 终结点。该方法是异步执行的，但在同一个实例中，不会与其他 `.connect()` 或 `.close()` 重复执行。
...
以 URL 的形式指定 SurrealDB 的终结点。如果 URL 路径的末尾不是 `/rpc`，则会自动添加。因此，如果您传递 `http://localhost:8000` 作为参数，它将连接到 `http://localhost:8000/rpc`。
...
`signal?: AbortSignal`
:   用于中断连接的终止信号。默认情况下，将设置一个在 15 秒后超时终止的信号。
...
无法在没有 `.close()` 断开连接的情况下连接到不同的终结点。注意，与先前建立连接的终结点的连接将被保留：
...
`.connect()` 不会在异步处理的上下文中同时执行，因此以下示例也将导致连接失败。通常情况下，无法确定哪个终结点先建立连接，因此错误消息可能因情况而异：
...
断开与 SurrealDB 的连接。如果尚未连接或已经断开连接，则不会抛出错误。该方法是异步执行的，但在同一个实例中，不会与 `.connect()` 或其他 `.close()` 重复执行。
...
`force?: boolean`
:   如果设置为 `true`，则将向 `Surreal` 对象的事件监听器发送终止信号。预期事件监听器即使处理不完整也会立即中止。
...
获取连接信息。连接信息始终包含内部信息的副本。因此，连接信息包含 `URL` 对象，但直接修改它不会更改 `Surreal` 对象内部存储的终结点信息。如果尚未请求连接，则为 `undefined`。
...
如果已建立连接，则返回连接信息。否则，返回 `undefined`。连接信息包含以下项目：
...
为了方便起见，即使尚未建立连接，它也是 `"closed"`（不是 `undefined`）。
...
当前选择的名空间。如果未选择名空间，则为 `null`。如果尚未建立连接，则为 `undefined`。
...
`"connecting" | "open" | "closing" | "closed"`
:   当连接信息发生更改时，或更改失败时发生的事件。

`rpc_${RpcMethod}_${number}`
:   双向通信中发生 RPC 响应时的事件。`RpcMethod` 例如 `ping` 或 `use` 等 PRC 方法名称。`number` 是双向通信中用于将请求/响应关联起来的标识符，它可以是 1 到 2^53-1 之间的整数。在 HTTP 中，它固定为 0。

`live_${string}`
:   当接收到实时查询的事件时发生的事件。目前仅在 WebSocket 协议中使用，在 HTTP 协议中不使用此事件。`string` 是实时查询的 UUID。
...
当事件发生时执行的函数。另请参阅 [任务发射器](/v2/api/utils/task-emitter#on)。传递给 `eventArgs` 的值因事件而异。
