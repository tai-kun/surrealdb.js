...
预备查询可以使类型推断生效，但需要手动设置推断的类型。

## 验证查询结果

将查询结果的验证器传递给 `.as` 的参数，可以使查询更加安全。以下示例使用 zod 来验证查询结果是否为预期值：
...
不依赖于 zod，可以使用 valibot 或通用函数来验证响应。

## 在查询中嵌入变量

`surql` 允许使用模板字符串编写查询，以便在代码中直观地嵌入值：
...
槽位必须包含变量名，并且可以使用 `.as()` 方法在类型级别上对变量进行约束。与 `.as()` 相似，可以将验证函数作为 `.as()` 的参数传递，用于验证传入的值。在上面的示例中，变量名 `id` 的槽位必须是表名为 `"user"` 的记录 ID。

槽位参数可以包含变量名，并使用默认值进行初始化。如果在执行时省略了槽位的变量名，将使用该默认值。在上面的示例中，变量名 `age` 的槽位使用默认值 `42` 进行初始化。

除了 `.as()` 之外，槽位还可以使用 `.rename()`、`.default()`、`.optional()`、`.required()` 等方法。

如果预定义的查询包含在执行时必须指定变量的槽位，但没有指定，则查询将在执行时被 `SurrealTypeError` 拒绝。如果在 TypeScript 中正确进行了类型推断，则可以在执行之前通过类型级别检查发现错误。例如，如果省略了必须的变量并使用 `tsc` 进行类型检查，则会显示以下错误：
