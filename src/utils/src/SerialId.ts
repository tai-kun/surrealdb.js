...
export default class SerialId {
   ...
   * @param max 最大値。
   */
  constructor(max: number | undefined = MAX_SAFE_INT) {
    if (Number.isSafeInteger(max) && max > 0) {
      this.#max = max;
    } else {
      throw new SurrealRangeError(
