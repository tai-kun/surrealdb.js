...
const TableSchema = z.custom<Table>(isTable);
const ThingSchema = z.custom<Thing>(isThing);
...
export default class ZodValidator implements Validator {
  parseRpcResult: Validator["parseRpcResult"] = (x, c) =>
    RpcResultMapping[c.request.method].parse(x);

  parseLiveResult: Validator["parseLiveResult"] = x =>
    LiveResult(
      RecordData,
      z.array(Patch(z.unknown())),
      z.union([z.string().uuid(), UuidSchema]),
    )
      .parse(x);

  parseRpcRequest: Validator["parseRpcRequest"] = x => RpcRequest.parse(x);

  parseRpcResponse: Validator["parseRpcResponse"] = x =>
    RpcResponse(z.unknown()).parse(x);

  parseIdLessRpcResponse: Validator["parseIdLessRpcResponse"] = x =>
    IdLessRpcResponse(z.unknown()).parse(x);
}
